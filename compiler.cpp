...
std::string RTError::as_string() {
  ...
  result += "File " + this->pos.Filename + ", line " +
            std::to_string(this->pos.line + 1) + ", col " +
            std::to_string(this->pos.column + 1) + "\n\n";
  ...
  return std::visit(
      [](auto &&arg) -> AnyNode {
        ...
        if constexpr (std::is_same_v<T, NumberNode>) {
          ...
          return RefVarDeclNode(arg.type_tok, arg.var_name_tok, arg.target_tok,
                                arg.pos);
          ...
          return std::make_shared<PropertyAccessNode>(
              clone_node(*(arg->base)), arg->base_name_tok, arg->property_name);
        } else if constexpr (std::is_same_v<T,
          ...
          return std::make_unique<ArrayAccessNode>(clone_node(arg->base),
      ...
      [](auto &&arg) -> std::string {
        ...
        if constexpr (std::is_same_v<T, NumberNode>) {
          ...
          return arg->print();
                                 ...
                                 T, std::unique_ptr<ArrayLiteralNode>>) {
          return arg->print();
        } else if constexpr (std::is_same_v<T,
                                            std::unique_ptr<ArrayAccessNode>>) {
          return arg->print();
...
std::string VarAssignNode::print() const {
  return "(" + this->type_tok.print() + " " + this->var_name_tok.print() + " " +
         printAny(this->value_node) + ")";
}
std::string VarAccessNode::print() const {
  return "(" + this->var_name_tok.print() + ")";
}
NumberNode::NumberNode(Token tok) { this->tok = tok; }
std::string CharNode::print() const { return this->tok.print(); }
std::string NumberNode::print() const { return this->tok.print(); }
std::string BinOpNode::print() const {
  return "(" + printAny(left_node) + " " + op_tok.print() + " " +
         printAny(right_node) + ")";
}
...
std::string StringNode::print() const { return "(" + this->tok.print() + ")"; }
StringNode::StringNode(Token tok) { this->tok = tok; }
BoolNode::BoolNode(Token tok) { this->tok = tok; }
std::string BoolNode::print() const { return "(" + this->tok.print() + ")"; }
QBoolNode::QBoolNode(Token tok) { this->tok = tok; }
std::string QBoolNode::print() const { return "(" + this->tok.print() + ")"; }
...
// /////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////
AnyNode ParseResult::reg_node(AnyNode res) { return res; }
...
Prs Parser::qif_expr() {
  ...
  if (this->current_tok.type != TokenType::RBRACE) {
    res.failure(std::make_unique<InvalidSyntaxError>("QC-S004: Expected '}'",
  ...
  auto then_branch =
      std::make_unique<StatementsNode>(std::move(then_stmts), true);
  if (res.error) {
    return res.to_prs();
  }

  std::vector<std::pair<AnyNode, std::unique_ptr<StatementsNode>>>
      qelif_branches;

  while (this->current_tok.type == TokenType::KEYWORD &&
         this->current_tok.value == "qelif") {
    this->advance();
    if (this->current_tok.type != TokenType::LPAREN) {
      res.failure(std::make_unique<InvalidSyntaxError>(
    ...
    if (this->current_tok.type != TokenType::RPAREN) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S006: Expected ')' after qelif condition",
          this->current_tok.pos));
      return res.to_prs();
    }
    this->advance();
    ...
    if (this->current_tok.type != TokenType::LBRACE) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S007: Expected '{' after qelif condition",
          this->current_tok.pos));
      return res.to_prs();
    }
    ...
    auto qelif_body =
        std::make_unique<StatementsNode>(std::move(qelif_stmts), true);
    if (res.error)
      return res.to_prs();
  ...
  if (this->current_tok.type == TokenType::KEYWORD &&
    ...
    qelse_branch =
        std::make_unique<StatementsNode>(std::move(qelse_stmts), true);
    if (res.error)
      return res.to_prs();
  }
  return res.success(std::make_unique<QIfNode>(
      std::nullopt, std::move(condition), std::move(then_branch),
      std::move(qelif_branches), std::move(qelse_branch)));
}
Prs Parser::if_expr() {
  ...
  if (!(this->current_tok.type == TokenType::KEYWORD &&
    ...
    res.failure(std::make_unique<InvalidSyntaxError>("QC-S009: Expected 'if'",
                                                     this->current_tok.pos));
    return res.to_prs();
  }
  this->advance();
  ...
  if (this->current_tok.type != TokenType::LPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S010: Expected '(' after 'if'", this->current_tok.pos));
    return res.to_prs();
  }
  this->advance();
    ...
    if (this->current_tok.type == TokenType::KEYWORD &&
      ...
      if (tok.value == "const") {
        ...
        if (tok.type != TokenType::KEYWORD) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S011: Expected type after 'const' in if-init",
              this->current_tok.pos));
          return res.to_prs();
        }
      }
      ...
      if (this->current_tok.type != TokenType::IDENTIFIER) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S012: Expected identifier in if-init", this->current_tok.pos));
        return res.to_prs();
      }
      ...
      if (this->current_tok.type == TokenType::EQ) {
        ...
        if (is_const) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S013: const variables must be initialized in if-init",
              var_name.pos));
          return res.to_prs();
        }
      ...
      if (this->current_tok.type != TokenType::SEMICOLON) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S014: Expected ';' after if-init declaration",
            this->current_tok.pos));
        return res.to_prs();
      }
      this->advance();

      init_node = AnyNode{std::make_unique<VarAssignNode>(
      ...
      if (this->current_tok.type != TokenType::SEMICOLON) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S015: Expected ';' after if-init expression",
            this->current_tok.pos));
        return res.to_prs();
      }
      this->advance();
  ...
  if (this->current_tok.type != TokenType::RPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S016: Expected ')' after condition", this->current_tok.pos));
    return res.to_prs();
  }
  this->advance();

  std::unique_ptr<StatementsNode> then_branch;
  if (!parse_block_into(then_branch, res))
    return res.to_prs();
  ...
  while (this->current_tok.type == TokenType::KEYWORD &&
    ...
    if (this->current_tok.type == TokenType::KEYWORD &&
      ...
      if (this->current_tok.type != TokenType::LPAREN) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S017: Expected '(' after 'else if'", this->current_tok.pos));
        return res.to_prs();
      }
      this->advance();
      ...
      if (this->current_tok.type != TokenType::RPAREN) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S018: Expected ')' after 'else if' condition",
            this->current_tok.pos));
        return res.to_prs();
      }
      this->advance();
      std::unique_ptr<StatementsNode> elif_block;
      if (!parse_block_into(elif_block, res))
        return res.to_prs();
      elifs.emplace_back(std::move(elif_cond), std::move(elif_block));
      continue;
    } else {
      if (!parse_block_into(else_branch, res))
        return res.to_prs();
      break;
    }
  }
  ...
  auto ifnode = std::make_unique<IfNode>(
      std::move(init_node), std::move(condition), std::move(then_branch),
      std::move(elifs), std::move(else_branch));
...
Prs Parser::try_catch_expr() {
  ...
  if (!(this->current_tok.type == TokenType::KEYWORD &&
    ...
    res.failure(std::make_unique<InvalidSyntaxError>("QC-S030: Expected 'try'",
                                                     this->current_tok.pos));
    return res.to_prs();
  }
  ...
  if (!(this->current_tok.type == TokenType::KEYWORD &&
    ...
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S031: Expected 'catch' after try block", this->current_tok.pos));
    return res.to_prs();
  }
  this->advance();
  if (this->current_tok.type != TokenType::LPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S032: Expected '(' after 'catch'", this->current_tok.pos));
    return res.to_prs();
  }
  this->advance();
  if (this->current_tok.type != TokenType::KEYWORD &&
    ...
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S033: Expected type in catch declaration", this->current_tok.pos));
    return res.to_prs();
  }
  ...
  if (this->current_tok.type != TokenType::IDENTIFIER) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S034: Expected variable name in catch declaration",
        this->current_tok.pos));
    return res.to_prs();
  }
  ...
  if (this->current_tok.type != TokenType::RPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S035: Expected ')' after catch variable", this->current_tok.pos));
    return res.to_prs();
  }
  this->advance();
  std::unique_ptr<StatementsNode> catch_body;
  if (!parse_block_into(catch_body, res))
    return res.to_prs();
  auto try_catch_node = std::make_unique<TryCatchNode>(
      std::move(try_body), catch_var, catch_type, std::move(catch_body),
      try_tok, try_tok.pos);
...
Prs Parser::switch_stmt() {
  ...
  if (current_tok.type != TokenType::LPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S020: Expected '(' after 'switch'", current_tok.pos));
    return res.to_prs();
  }
  advance();
  ...
  if (current_tok.type != TokenType::RPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S021: Expected ')' after switch expression", current_tok.pos));
    return res.to_prs();
  }
  advance();
  ...
  if (current_tok.type != TokenType::LBRACE) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S022: Expected '{' after switch(...)", current_tok.pos));
    return res.to_prs();
  }
  advance();
  ...
  while (this->current_tok.type != TokenType::RBRACE &&
    ...
    while (this->current_tok.type == TokenType::KEYWORD &&
      ...
      if (current_tok.value == "default") {
        ...
        if (current_tok.type != TokenType::COLON) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S023: Expected ':' after case label", current_tok.pos));
          return res.to_prs();
        }
        advance();
    ...
    if (!saw_label) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S024: Expected 'case' or 'default' inside switch",
          current_tok.pos));
      return res.to_prs();
    }
  ...
  if (current_tok.type != TokenType::RBRACE) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S025: Expected '}' after switch body", current_tok.pos));
    return res.to_prs();
  }
  advance();
...
Prs Parser::qswitch_stmt() {
  ...
  while (this->current_tok.type == TokenType::KEYWORD &&
    ...
    if (this->current_tok.type != TokenType::IDENTIFIER) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S029: Expected case label (t, f, n, or b)",
          this->current_tok.pos));
      return res.to_prs();
    }
    ...
    auto case_body =
        std::make_unique<StatementsNode>(std::move(case_stmts), true);
    ...
    if (case_label == "t") {
      ...
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S030: Invalid case label (must be t, f, n, or b)",
          this->current_tok.pos));
      return res.to_prs();
    }
  }
  ...
  return res.success(std::make_unique<QSwitchNode>(
      std::move(value), std::move(case_t), std::move(case_f), std::move(case_n),
      std::move(case_b)));
}
Prs Parser::while_stmt() {
  ...
  if (current_tok.type != TokenType::LPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S033: Expected '(' after 'while'", current_tok.pos));
    return res.to_prs();
  }
  advance();
  ...
  if (current_tok.type != TokenType::RPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S034: Expected ')' after while condition", current_tok.pos));
    return res.to_prs();
  }
  advance();

  std::unique_ptr<StatementsNode> body;
  if (!this->parse_block_into(body, res))
    return res.to_prs();
...
Prs Parser::for_stmt() {
  ...
  if (!(this->current_tok.type == TokenType::KEYWORD &&
    ...
    res.failure(std::make_unique<InvalidSyntaxError>("QC-S035: Expected 'for'",
                                                     current_tok.pos));
    return res.to_prs();
  }
  this->advance();
  ...
  if (this->current_tok.type != TokenType::LPAREN) {
    res.failure(std::make_unique<InvalidSyntaxError>(
        "QC-S036: Expected '(' after 'for'", current_tok.pos));
    return res.to_prs();
  }
  this->advance();
  ...
  if (this->current_tok.type != TokenType::SEMICOLON) {
    if (this->current_tok.type == TokenType::KEYWORD &&
      ...
      if (tok.value == "const") {
        ...
        if (tok.type != TokenType::KEYWORD) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S011:  Expected type after 'const' in for-init",
              current_tok.pos));
          return res.to_prs();
        }
      }
      ...
      if (current_tok.type == TokenType::EQ) {
        ...
        if (is_const) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S039: const variables must be initialized in for-init",
              var_name.pos));
          return res.to_prs();
        }
        value = default_value_for_type(type_tok, var_name.pos);
      ...
      init = AnyNode{std::make_unique<VarAssignNode>(
          is_const, type_tok, var_name, std::move(value))};
    } else {
      AnyNode expr_init = res.reg(this->assignment_expr());
      if (res.error)
  ...
  if (current_tok.type == TokenType::SEMICOLON) {
    condition =
        AnyNode{BoolNode(Token(TokenType::BOOL, "true", current_tok.pos))};
  } else {
...
Prs Parser::call(AnyNode node_to_call) {
  ...
  return res.success(
      std::make_unique<CallNode>(std::move(node_to_call), std::move(args)));
}
  ...
  if (this->current_tok.type != TokenType::RBRACKET) {
    ...
    while (this->current_tok.type == TokenType::COMMA) {
      ...
      if (this->current_tok.type == TokenType::AT) {
        ...
        elements.push_back(
            std::make_unique<SpreadNode>(std::move(spread_expr)));
      } else {
        AnyNode elem = res.reg(this->logical_or());
        if (res.error)
          return res.to_prs();
...
Prs Parser::atom() {
  ...
  if (tok.type == TokenType::LBRACE) {
    ...
    if (this->current_tok.type == TokenType::COLON) {
      ...
      if (this->current_tok.type != TokenType::RBRACE) {
        res2.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S047: Expected '}' at end of map literal",
            this->current_tok.pos));
        return res2.to_prs();
      }
      this->advance();
      ...
      if (this->current_tok.type != TokenType::RBRACE) {
        res2.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S048: Expected '}' in initializer list",
            this->current_tok.pos));
        return res2.to_prs();
      }
      this->advance();
  ...
  if (tok.type == TokenType::INT || tok.type == TokenType::FLOAT ||
      tok.type == TokenType::DOUBLE || tok.type == TokenType::ADDR_T) {
    this->advance();
    ...
    while (this->current_tok.type == TokenType::SCOPE) {
      ...
      if (this->current_tok.type != TokenType::IDENTIFIER) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-N001: Expected identifier or namespace name after '::'",
            this->current_tok.pos));
        return res.to_prs();
      }
    ...
    while (this->current_tok.type == TokenType::DOT ||
      ...
      if (this->current_tok.type == TokenType::ARROW) {
        ...
        auto deref = std::make_unique<UnaryOpNode>(
            Token(TokenType::MUL, "*", property_name.pos), std::move(base));
        ...
        if (this->current_tok.type == TokenType::LPAREN) {
          ...
          if (this->current_tok.type != TokenType::RPAREN) {
            res.failure(std::make_unique<InvalidSyntaxError>(
                "Expected ')' after method arguments", this->current_tok.pos));
            return res.to_prs();
          }
          this->advance();
        ...
        if (this->current_tok.type == TokenType::LPAREN) {
          ...
          base = std::make_shared<PropertyAccessNode>(std::move(base), ident,
                                                      property_name);
        }
    ...
    if (this->current_tok.type == TokenType::RPAREN) {
      ...
      while (this->current_tok.type == TokenType::DOT) {
        ...
        if (auto unary = std::get_if<std::unique_ptr<UnaryOpNode>>(&base)) {
          if ((*unary)->op_tok.type == TokenType::MUL) {
            if (auto var = std::get_if<std::unique_ptr<VarAccessNode>>(
                    &(*unary)->node)) {
              base_name_tok = (*var)->var_name_tok;
            } else {
              base_name_tok =
      ...
      res.failure(std::make_unique<InvalidSyntaxError>("QC-S050: Expected ')'",
                                                       this->current_tok.pos));
      return res.to_prs();
    }
    ...
    for (size_t i = 0; i < exprs.size(); ++i) {
      ...
      result = std::make_unique<BinOpNode>(std::move(result),
                                           Token(TokenType::PLUS, "+", tok.pos),
                                           std::move(expr_node), true);

      if (i + 1 < parts.size()) {
        result = std::make_unique<BinOpNode>(
            std::move(result), Token(TokenType::PLUS, "+", tok.pos),
            StringNode(Token(TokenType::STRING, parts[i + 1], tok.pos)), true);
      }
    }
  ...
  if (tok.type == TokenType::KEYWORD && tok.value == "fn") {
    ...
    if (this->current_tok.type == TokenType::ARROW) {
      ...
      if (this->current_tok.type != TokenType::KEYWORD &&
          !find_type(this->current_tok.value)) {
        res.failure(std::make_unique<InvalidSyntaxError>(
      ...
      while (this->current_tok.type == TokenType::COMMA) {
        ...
        if (this->current_tok.type != TokenType::KEYWORD) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S053: Expected return type after ','",
              this->current_tok.pos));
          return res.to_prs();
        }
...
Prs Parser::power() {
  ...
  if (this->current_tok.type == TokenType::POWER) {
    ...
    if (res.error)
      return res.to_prs();
    left =
        std::make_unique<BinOpNode>(std::move(left), op_tok, std::move(right));
  }
...
Prs Parser::factor() {
  ...
  if (tok.type == TokenType::PLUS || tok.type == TokenType::MINUS ||
      tok.type == TokenType::BITWISE_NOT || tok.type == TokenType::NOT ||
      tok.type == TokenType::QNOT || tok.type == TokenType::AMPERSAND ||
    ...
    if (res.error)
      return res.to_prs();
    return res.success(
        std::make_unique<UnaryOpNode>(tok, std::move(factor_node)));
  }
...
Prs Parser::logical_and() {
  ...
  while (this->current_tok.type == TokenType::AND ||
         this->current_tok.type == TokenType::AMPERSAND) {
    Token op_tok = this->current_tok;
    this->advance();
...
Prs Parser::logical_or() {
  ...
  while (this->current_tok.type == TokenType::OR ||
         this->current_tok.type == TokenType::XOR ||
         this->current_tok.type == TokenType::PIPE ||
...
Prs Parser::qin_expr() {
  ...
  while (current_tok.type == TokenType::RSHIFT) {
    ...
    if (!std::holds_alternative<std::unique_ptr<VarAccessNode>>(right)) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S055: Right-hand side of >> must be a variable", op_tok.pos));
      return res.to_prs();
    }
...
Prs Parser::bin_op(std::function<Prs()> func,
  ...
  while (std::find(ops.begin(), ops.end(), this->current_tok.type) !=
         ops.end()) {
    Token op_tok = this->current_tok;
    this->advance();
...
Prs Parser::return_stmt() {
  ...
  if (this->current_tok.type == TokenType::SEMICOLON) {
    this->advance();
    return res.success(
        std::make_unique<ReturnNode>(std::monostate{}, start_pos));
  }
  ...
  if (values.size() == 1) {
    return res.success(
        std::make_unique<ReturnNode>(std::move(values[0]), start_pos));
  }

  return res.success(
      std::make_unique<MultiReturnNode>(std::move(values), start_pos));
}
Prs Parser::assignment_expr() {
  ...
  if (this->current_tok.type == TokenType::EQ ||
    ...
    bool is_array_access =
        std::holds_alternative<std::unique_ptr<ArrayAccessNode>>(left);
    bool is_prop =
        std::holds_alternative<std::shared_ptr<PropertyAccessNode>>(left);
    bool is_deref =
        std::holds_alternative<std::unique_ptr<UnaryOpNode>>(left) &&
        std::get<std::unique_ptr<UnaryOpNode>>(left)->op_tok.type ==
            TokenType::MUL;
    if (!is_var && !is_array_access && !is_prop && !is_deref) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S056: Left side of assignment must be a variable, struct field, "
    ...
    if (is_prop) {
      ...
      switch (op_tok.type) {
      case TokenType::EQ:
        binop_type = TokenType::EQ;
        break;
      case TokenType::PLUS_EQ:
        binop_type = TokenType::PLUS;
        break;
      case TokenType::MINUS_EQ:
        binop_type = TokenType::MINUS;
        break;
      case TokenType::MUL_EQ:
        binop_type = TokenType::MUL;
        break;
      case TokenType::DIV_EQ:
        binop_type = TokenType::DIV;
        break;
      case TokenType::MOD_EQ:
        binop_type = TokenType::MOD;
        break;
      default:
        res.failure(std::make_unique<InvalidSyntaxError>(
      ...
      if (binop_type == TokenType::EQ) {
        return res.success(std::make_unique<FieldAssignNode>(
            std::move(lhsBase), field, std::move(right)));
      }
      AnyNode rhsBase = clone_node(*(prop->base));

      auto readProp = std::make_shared<PropertyAccessNode>(
          std::move(rhsBase), prop->base_name_tok, field);
      AnyNode readPropNode = AnyNode{std::move(readProp)};
      Token bin_tok(binop_type, get_token_name(binop_type), op_tok.pos);
      AnyNode binExpr = AnyNode{std::make_unique<BinOpNode>(
          std::move(readPropNode), bin_tok, std::move(right))};
      return res.success(std::make_unique<FieldAssignNode>(
          std::move(lhsBase), field, std::move(binExpr)));
    }
    if (is_array_access) {
      if (op_tok.type != TokenType::EQ) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S058: Compound assignment (+=, -=, etc.) not supported for "
            "array/map access",
            op_tok.pos));
        return res.to_prs();
      }
      ...
      return res.success(
          std::make_unique<ArrayAssignNode>(std::move(left), std::move(right)));
    }

    return res.success(std::make_unique<AssignExprNode>(std::move(left), op_tok,
                                                        std::move(right)));
  }
...
Parameter Parser::parse_parameter(bool type_only = false) {
  Parameter p;
  if (this->current_tok.value == "fn" &&
      this->current_tok.type == TokenType::KEYWORD) {
    p.type = this->current_tok;
    this->advance();

    if (this->current_tok.type != TokenType::LPAREN) {
      throw std::make_unique<InvalidSyntaxError>("Expected '(' after 'fn'",
                                                 this->current_tok.pos);
    }
    this->advance();
    ...
    if ((p.type.value == "list" || p.type.value == "map") &&
      ...
      if (this->current_tok.type != TokenType::MORE) {
        throw std::make_unique<InvalidSyntaxError>(
            "Expected '>' after a list/map type", this->current_tok.pos);
      }
      this->advance();
      p.type.value += ">";
    }
    ...
    while (this->current_tok.type == TokenType::LBRACKET) {
      this->advance();
      if (this->current_tok.type != TokenType::RBRACKET) {
        throw std::make_unique<InvalidSyntaxError>("Expected ']' after '['",
                                                   this->current_tok.pos);
      }
      this->advance();
      p.type.value += "[]";
...
Prs Parser::func_def_multi(std::vector<Token> return_types,
  ...
  if (this->current_tok.type == TokenType::ARROW) {
    ...
    if (this->current_tok.type == TokenType::KEYWORD ||
        this->user_types.count(this->current_tok.value) > 0) {
  ...
  while (true) {
    ...
    if (this->current_tok.type == TokenType::EOFT) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "Unexpected end of file in function body", this->current_tok.pos));
      return res.to_prs();
    }
    Prs st = this->statement();
...
Prs Parser::statement() {
  ...
  if (tok.type == TokenType::KEYWORD && tok.value == "foreach") {
    ...
    if (this->current_tok.type != TokenType::LBRACE) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-S003: Expected '{' to start foreach body",
          this->current_tok.pos));
      return res.to_prs();
    }
    this->advance();
  ...
  if (tok.type == TokenType::KEYWORD && tok.value == "class") {
    ...
    if (this->current_tok.type != TokenType::IDENTIFIER) {
      res.failure(std::make_unique<InvalidSyntaxError>("Expected class name",
                                                       this->current_tok.pos));
      return res.to_prs();
    }
    ...
    while (this->current_tok.type != TokenType::RBRACE &&
      ...
      if (this->current_tok.type != TokenType::KEYWORD &&
          this->current_tok.type != TokenType::IDENTIFIER) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "Expected type or constructor in class body",
            this->current_tok.pos));
        return res.to_prs();
      }
      ...
      auto parse_one_type_into = [&](Token &out_tok) -> bool {
        ...
        if (this->current_tok.type == TokenType::IDENTIFIER) {
          ...
          while (this->current_tok.type == TokenType::SCOPE) {
            ...
            if (this->current_tok.type != TokenType::IDENTIFIER) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-N001: Expected identifier after '::'",
                  this->current_tok.pos));
              return false;
            }
            field_type += "::" + this->current_tok.value;
          ...
          if (base_type.value == "list" &&
            ...
            if (this->current_tok.type != TokenType::RBRACKET) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-S061: Expected ']' after '[' in list type",
                  this->current_tok.pos));
              return false;
            }
            this->advance();
            ...
            if (this->current_tok.type != TokenType::LESS) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-S066: Expected '<' after map for key type",
                  this->current_tok.pos));
              return false;
            }
            this->advance();
            ...
            if (this->current_tok.type != TokenType::COMMA) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-S068: Expected ',' between key and value type in map",
                  this->current_tok.pos));
              return false;
            }
            this->advance();
            ...
            if (this->current_tok.type != TokenType::KEYWORD &&
                this->current_tok.type != TokenType::IDENTIFIER) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-S069: Expected value type in map",
                  this->current_tok.pos));
              return false;
            }
            ...
            if (this->current_tok.type != TokenType::MORE) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-S070: Expected '>' after map value type",
                  this->current_tok.pos));
              return false;
            }
            this->advance();
      ...
      if (this->current_tok.type == TokenType::IDENTIFIER &&
        ...
        res.failure(std::make_unique<InvalidSyntaxError>(
            "Expected method or field name after type(s)",
            this->current_tok.pos));
        return res.to_prs();
      }
      if (this->current_tok.type == TokenType::LPAREN) {
        ...
        auto m_pr =
            this->func_def_multi(type_list, std::make_optional(name_tok));
      ...
      while (this->current_tok.type == TokenType::LBRACKET) {
        ...
        if (this->current_tok.type != TokenType::RBRACKET) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-S061: Expected ']' after '[' in array",
              this->current_tok.pos));
          return res.to_prs();
        }
        this->advance();
  ...
  if (tok.type == TokenType::KEYWORD && tok.value == "struct") {
    ...
    while (this->current_tok.type != TokenType::RBRACE &&
      ...
      if (this->current_tok.type != TokenType::SEMICOLON) {
        res.failure(
            std::make_unique<MissingSemicolonError>(this->current_tok.pos));
        return res.to_prs();
      }
      this->advance();
    ...
    if (user_types.contains(struct_name.value)) {
      res.failure(std::make_unique<InvalidSyntaxError>(
          "QC-UT01: Redefinition of struct '" + struct_name.value + "'",
          struct_name.pos));
      return res.to_prs();
    }
    UserTypeInfo info;
  ...
  if (tok.type == TokenType::KEYWORD && tok.value == "type") {
    ...
    auto parse_type_atom = [&](Token first_tok) -> UnionMember {
      ...
      case TokenType::STRING:
        return UnionMember{"string:\"" + first_tok.value + "\""};
      case TokenType::INT:
        return UnionMember{"int:" + first_tok.value};
      case TokenType::FLOAT:
        return UnionMember{"float:" + first_tok.value};
      case TokenType::DOUBLE:
        return UnionMember{"double:" + first_tok.value};
      case TokenType::ADDR_T:
        return UnionMember{"addr_t:" + first_tok.value};
      case TokenType::CHAR:
        return UnionMember{"char:" + first_tok.value};
      case TokenType::BOOL:
        return UnionMember{"bool:" + first_tok.value};
      case TokenType::QBOOL:
        return UnionMember{"qbool:" + first_tok.value};
      default:
        break;
      }
    ...
    auto is_type_or_literal_token = [&](TokenType tt) {
      return tt == TokenType::STRING || tt == TokenType::IDENTIFIER ||
             tt == TokenType::KEYWORD || tt == TokenType::INT ||
             tt == TokenType::FLOAT || tt == TokenType::DOUBLE ||
             tt == TokenType::ADDR_T || tt == TokenType::BOOL ||
             tt == TokenType::QBOOL || tt == TokenType::CHAR;
    };
  ...
  if (tok.type == TokenType::KEYWORD && tok.value == "enum") {
    ...
    while (this->current_tok.type != TokenType::RBRACE &&
      ...
      if (this->current_tok.type != TokenType::EQ) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S082: Expected '=' after enum member name '" +
                member_name.value + "'",
            this->current_tok.pos));
        return res.to_prs();
      }
  ...
  if (tok.type == TokenType::KEYWORD) {
    ...
    if (type_tok.value == "map" && this->current_tok.type == TokenType::LESS) {
      ...
      if (this->current_tok.type != TokenType::KEYWORD) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S069: Expected value type in map<K, V>",
            this->current_tok.pos));
        return res.to_prs();
      }
      ...
      if (this->current_tok.type == TokenType::IDENTIFIER) {
        ...
        if (this->peek().type == TokenType::EQ ||
          ...
          if (this->current_tok.type == TokenType::EQ) {
            ...
            if (this->current_tok.type != TokenType::LBRACE) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "QC-S003: Expected '{' for map initialization",
                  this->current_tok.pos));
              return res.to_prs();
            }
            this->advance();
            ...
            if (this->current_tok.type != TokenType::RBRACE) {
              res.failure(std::make_unique<InvalidSyntaxError>(
                  "Expected '}' after map initialization",
                  this->current_tok.pos));
              return res.to_prs();
            }
            this->advance();
    ...
    if (type_tok.value == "list" && this->current_tok.type == TokenType::LESS) {
      ...
      if (this->current_tok.type != TokenType::KEYWORD) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "QC-S060: Expected element type in list<T>",
            this->current_tok.pos));
        return res.to_prs();
      }
      ...
      type_tok = Token(TokenType::KEYWORD, "list<" + elem_type.value + ">",
                       type_tok.pos);
      is_list = true;
    }
    ...
    if (type_tok.value.find("[][]") != std::string::npos) {
      if (this->current_tok.type != TokenType::IDENTIFIER) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "Multi-dimensional types can only be used in function returns",
            this->current_tok.pos));
        return res.to_prs();
      }
    ...
    if (this->current_tok.type == TokenType::COMMA) {
      ...
      res.failure(std::make_unique<InvalidSyntaxError>(
          "Invalid syntax: use 'int x, string y = ...' for multi-variable "
          "declarations",
          name_tok.pos));
      return res.to_prs();
    }
    ...
    if (this->current_tok.type == TokenType::COMMA) {
      while (this->current_tok.type == TokenType::COMMA) {
        ...
        if (this->current_tok.value == "list" &&
            this->index + 1 < this->tokens.size() &&
      ...
      if (this->current_tok.type != TokenType::EQ) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "Expected '=' in multi-variable declaration",
            this->current_tok.pos));
        return res.to_prs();
      }
      this->advance();
      ...
      if (return_types.size() != var_names.size()) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "Number of types must match number of variables",
            var_names[0].pos));
        return res.to_prs();
      }
    ...
    if (is_list) {
      ...
      if (this->current_tok.type == TokenType::EQ) {
        ...
        if (is_const) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-T007: const variables and references must be initialized",
              name_tok.pos));
          return res.to_prs();
        }
        value = std::make_unique<ArrayLiteralNode>(std::vector<AnyNode>{},
                                                   name_tok.pos);
      }
    ...
    if (is_array) {
      ...
      return res.success(std::make_unique<ArrayDeclNode>(
          is_const, type_tok, name_tok, std::move(value), dimensions,
    ...
    if (this->current_tok.type == TokenType::EQ) {
      ...
      if (is_reference) {
        if (this->current_tok.type != TokenType::IDENTIFIER) {
          res.failure(std::make_unique<InvalidSyntaxError>(
              "QC-R001: Cannot assign an expression to a reference.",
              var_names[0].pos));
          return res.to_prs();
        }
        ...
        this->advance();
        auto ref_node =
            RefVarDeclNode(type_tok, var_names[0], target, type_tok.pos);
      ...
      if (is_const || is_reference) {
        res.failure(std::make_unique<InvalidSyntaxError>(
            "const variables and references must be initialized",
            var_names[0].pos));
        return res.to_prs();
      }
  ...
  if (tok.type == TokenType::IDENTIFIER) {
    ...
    if (maybe_qualified.has_value()) {
      ...
      if (is_type) {
          ...
          if (peek_idx < tokens.size()) {
            ...
            if (peek.type == TokenType::KEYWORD) {
              ...
              if (peek_qual.has_value()) {
                ...
                is_multi_return = (find_type(base_name) != nullptr ||
        ...
        if (this->current_tok.type == TokenType::COMMA) {
          ...
          while (this->current_tok.type == TokenType::COMMA) {
            ...
            if (this->current_tok.type == TokenType::KEYWORD) {
              ...
              if (qual_name.has_value()) {
                ...
                if (find_type(base_name) != nullptr ||
                    is_known_type(*qual_name)) {
          ...
          if (this->current_tok.type != TokenType::EQ) {
            res.failure(std::make_unique<InvalidSyntaxError>(
                "Expected '=' in multi-var declaration",
                this->current_tok.pos));
            return res.to_prs();
          }
          this->advance();
          ...
          return res.success(std::make_unique<MultiVarDeclNode>(
              false, var_types, var_names, std::move(value)));
        }
        if (this->current_tok.type == TokenType::LPAREN) {
          auto func_def = res.reg(this->func_def_multi(return_types, name_tok));
          if (res.error)
    ...
    size_t next_i = index + 1;
    if (next_i < tokens.size() && (tokens[next_i].type == TokenType::EQ ||
        ...
        [&has_main, &main_func_ptr](auto &&arg) -> AnyNode {
          ...
          if constexpr (std::is_same_v<T, std::shared_ptr<FuncDefNode>>) {
            if (arg->name_tok.has_value() &&
                arg->name_tok->value == entrypointName) {
              if (arg->return_types.empty() ||
                  arg->return_types[0].value != "int") {
                std::string actual = arg->return_types.empty()
                                         ? "void"
                                         : arg->return_types[0].value;
                throw InvalidSyntaxError(
                    "the entrypoint must return int, not " + actual,
                    arg->return_types.empty() ? Position()
                                              : arg->return_types[0].pos);
              }
              if (!arg->params.empty() &&
                  arg->params.front().type.value != "string[]") {
                throw InvalidSyntaxError(
                    "the entrypoint must have no parameters or take a array of "
                    "strings.",
                    arg->return_types.empty() ? Position()
                                              : arg->return_types[0].pos);
              }
  ...
  for (auto &[name, ut] : user_types) {
    if (ut.kind == UserTypeKind::Class && !ut.baseClassName.empty()) {
      ...
      if (baseKey.find("::") == std::string::npos) {
        ...
        if (!found) {
          throw InvalidSyntaxError(
              "Base class '" + ut.baseClassName + "' not found", Position());
        }
      }
      ...
      if (it == user_types.end() || it->second.kind != UserTypeKind::Class) {
        throw InvalidSyntaxError(
            "class inherits from non-class or non-existent object", Position());
      }
    }
  }
  return Aer{std::make_unique<StatementsNode>(std::move(stmts), true), nullptr,
             std::move(this->user_types)};
}
//////////////////////////////////////////////////////////////////////////////////////////////
// VALUES
// ///////////////////////////////////////////////////////////////////////////////////
        ...
        if constexpr (std::is_same_v<T, BoolValue>) {
          ...
          if (!method) {
            this->errors.push_back(
                {RTError("QC-CLW1: Instance of '" + className +
          ...
          NumberVariant result = call_instance_method(
              inst, method, std::vector<NumberVariant>{}, Position());
          if (auto b = std::get_if<BoolValue>(&result)) {
            return b->value;
          }

          this->errors.push_back(
              {RTError("eval() on class '" + className + "' must return bool",
                       get_pos(result)),
               "Error"});
          return false;
        }
...
bool value_matches_union_member(const std::string &member_type,
  ...
  if (colon_pos != std::string::npos) {
    ...
    if (member_type == "auto" || actual_type == "auto")
      return true;
    ...
    if (lit_kind == "string" && actual_type == "string") {
      if (auto sv = std::get_if<StringValue>(&val)) {
        if (lit_val.size() >= 2 && lit_val.front() == '"' &&
            lit_val.back() == '"') {
    ...
    if (lit_kind == "char" && actual_type == "char") {
      if (auto cv = std::get_if<CharValue>(&val)) {
        ...
        if (lit_val.size() >= 3 && lit_val.front() == '\'' &&
            lit_val.back() == '\'') {
          char c = lit_val[1];
          return cv->value == c;
        }
...
void Context::set(const std::string &name, NumberVariant new_val,
  ...
  if (normalize_type(expected) != normalize_type(actual) &&
      expected != actual) {
    throw RTError("QC-T003: Type mismatch: cannot assign " + actual + " to " +
                      expected,
                  pos);
  }
...
NumberVariant def_value_for_type(const std::string &type_name) {
  ...
  if (type_name.size() >= 2 && type_name.find("[]") != std::string::npos) {
    ...
    auto arr =
        std::make_shared<ArrayValue>(elem_type, std::vector<NumberVariant>{});
    return arr;
  }
  ...
  return std::visit(
      [this](auto const &v) -> std::string {
        ...
        if constexpr (std::is_same_v<T, std::monostate>) {
          ...
          if (!method) {
            this->errors.push_back(
                {RTError("QC-CLW2: Instance of '" + className +
                             "' printed, but missing repr function. Printing "
                             "(reprless class)",
                         Position()),
                 "Warning"});
          ...
          this->errors.push_back(
              {RTError("repr() on class '" + className + "' must return string",
                       get_pos(result)),
               "Error"});
          return v->print();
        } else if constexpr (std::is_same_v<T, PointerValue>) {
  ...
  try {
    ...
    if (node->catch_var_type == "auto") {
      ...
      if (node->catch_var_type == "list<auto>") {
        ...
        if (strip(node->catch_var_type) == inner &&
            node->catch_var_type != inner) {
          type_matches = true;
        }
      }
    }
    ...
    if (type_matches) {
      context->push_scope();
      context->define(node->catch_var_name, node->catch_var_type, thrown_value,
                      false);
    ...
    if (node->catch_var_type == "string" || node->catch_var_type == "auto") {
      context->push_scope();
      context->define(node->catch_var_name, "string", StringValue(e.details),
                      false);
...
Interpreter::convert_array_to_struct(const std::shared_ptr<ArrayValue> &arr,
  ...
  if (arr->elements.size() != fields.size()) {
    this->errors.push_back(
        {RTError("Nested struct initializer size mismatch for '" + struct_type +
                     "'",
                 Position("", "", 0, 0, 0)),
         "Error"});
  }
  ...
  for (size_t i = 0; i < fields.size(); ++i) {
    ...
    if (context->user_types.contains(field.type)) {
      ...
      elemVal = convert_array_to_struct(*nestedArr, field.type, context);
               ...
               (field.type.size() > 5 && field.type.substr(0, 5) == "list<")) {

      auto arrVal = std::get_if<std::shared_ptr<ArrayValue>>(&elemVal);
      if (!arrVal) {
        this->errors.push_back(
            {RTError("Expected array initializer for list/array field '" +
                         field.name + "'",
                     Position("", "", 0, 0, 0)),
             "Error"});
      }
      ...
      if (expectedKey != actualKey || expectedValue != actualValue) {
        this->errors.push_back(
            {RTError("QC-T010: Type mismatch for map field '" + field.name +
                         "': expected " + field.type + ", got map<" +
    ...
    if (ut.kind == UserTypeKind::Struct) {
      ...
      if (auto svPtr = std::get_if<std::shared_ptr<StructValue>>(&value)) {
        ...
        if (value_type == declared_simple) {
          ...
          this->errors.push_back(
              {RTError("Cannot assign struct of type '" + (*svPtr)->type_name +
        ...
        value = newStruct;
                     ...
                     std::get_if<std::shared_ptr<ArrayValue>>(&value)) {

        if (arrPtr->get()->elements.size() != fields.size()) {
          this->errors.push_back(
              {RTError("Initializer list has " +
                           std::to_string(arrPtr->get()->elements.size()) +
        ...
        for (size_t i = 0; i < fields.size(); ++i) {
          ...
          if (nested_it != context->user_types.end() &&
            ...
            elemVal = Interpreter::convert_array_to_struct(*nestedArr,
                                                           field.type, context);
          } else if (field.type.size() >= 2 &&
                     field.type.substr(field.type.size() - 2) == "[]") {
            ...
            if (!arrVal) {
              this->errors.push_back(
                  {RTError("Expected array initializer for list field '" +
                               field.name + "'",
                           node->var_name_tok.pos),
                   "Error"});
            }
            ...
            auto listVal =
                std::make_shared<ListValue>(elemType, (*arrVal)->elements);
            elemVal = listVal;
            ...
            if (expectedKeyType != actualKeyType ||
                expectedValType != actualValType) {
              this->errors.push_back(
                  {RTError("QC-T010: Type mismatch for map field '" +
                               field.name + "': expected " + field.type +
                               ", got map<" + actualKeyType + "," +
                               actualValType + ">",
                           node->var_name_tok.pos),
                   "Error"});
            }
        ...
        this->errors.push_back({RTError("Expected struct initializer '{ ... }' "
      ...
      if (!ok) {
        this->errors.push_back(
            {RTError("QC-T004: Type mismatch: value of type " + valType +
      ...
      if (std::holds_alternative<VoidValue>(value)) {
        ...
        auto inst =
            std::make_shared<InstanceValue>(lookup_type, std::move(fields));
  ...
  if (declaredType != "auto") {
    ...
    if (declaredType == "float" && actualType == "float") {
      type_matches = true;
               ...
               (actualType == "float" || actualType == "double")) {
      type_matches = true;
    ...
    if (declaredType == "list<auto>") {
      if (actualType.find("list<") != std::string::npos ||
          actualType.find("[]") != std::string::npos) {
        type_matches = true;
      }
    ...
    if (!type_matches) {
      this->errors.push_back({RTError("QC-T003: Type mismatch: expected " +
                                          declaredType + ", got " + actualType,
                                      node->var_name_tok.pos),
                              "Error"});
    }
  }
...
NumberVariant Interpreter::operator()(std::unique_ptr<MapDeclNode> &node) {
  ...
  auto map_val =
      std::make_shared<MapValue>(node->key_type.value, node->value_type.value);
  ...
  std::string map_type =
      "map<" + node->key_type.value + ", " + node->value_type.value + ">";
  ...
  if (auto qb = std::get_if<QBoolValue>(&val)) {
    ...
    if (qb->tval && qsw.case_t) {
      for (auto &stmt : qsw.case_t->statements) {
        ...
        last = std::move(r.value);
        if (r.did_break)
          return ExecResult{std::move(last), true, false};
        if (r.did_continue)
          return ExecResult{std::move(last), false, true};
      }
    }
...
ExecResult Interpreter::exec_stmt_in_loop_or_switch(TryCatchNode &tcn) {
  try {
    ...
    if (type_matches) {
      context->push_scope();
      context->define(tcn.catch_var_name, tcn.catch_var_type, thrown_value,
                      false);
      ExecResult r = {};
      if (tcn.catch_body) {
    ...
    if (tcn.catch_var_type == "string" || tcn.catch_var_type == "auto") {
      ...
      context->define(tcn.catch_var_name, "string", StringValue(e.details),
                      false);
...
NumberVariant Interpreter::operator()(std::unique_ptr<ContinueNode> &node) {
  this->errors.push_back({RTError("QC-SW02: Unexpected 'continue' outside loop",
...
bool Interpreter::struct_has_field(const std::shared_ptr<StructValue> &sv,
  ...
  if (it == sv->fields.end())
    return false;
  if (std::holds_alternative<VoidValue>(it->second))
    return false;
...
size_t Interpreter::get_sizeof_type(const std::string &type) {
  ...
  if (base_type == "qbool")
    return 1;
  if (base_type == "short" || base_type == "short int")
    return 2;
  if (base_type == "int")
    return 4;
  if (base_type == "long" || base_type == "long int")
    return 8;
  if (base_type == "float")
    return 4;
  if (base_type == "double")
    return 8;
  if (base_type == "long double")
    return 16;
  ...
  if (base_type.find("[]") != std::string::npos)
    return 8;
  if (base_type.find("list<") != std::string::npos)
    return 8;
  if (base_type.find("map<") != std::string::npos)
    return 8;
  return 0;
}
NumberVariant Interpreter::operator()(std::unique_ptr<CallNode> &node) {
  ...
  if (std::holds_alternative<std::unique_ptr<VarAccessNode>>(
    ...
    if (func_name == "qout") {
      ...
      for (size_t i = 0; i < val.size(); i++) {
        ...
        if (i >= val.size()) {
          this->errors.push_back(
              {RTError("QC-Q003: invalid formatter", get_pos(target_val)),
               "Error"});
          break;
        }
        bool zero_pad = false;
        ...
        case 'i':
          ...
          if (args.size() - 1 < current_arg) {
            ...
            ss << is_intVal->value;
            to_print += ss.str();
          } else if (auto is_addrtVal =
                         std::get_if<Number<uintptr_t>>(&args[current_arg])) {
            std::stringstream ss;
            if (zero_pad)
              ss << std::setfill('0');
            if (width >= 0)
        ...
        case 's':
          ...
          if (current_arg >= args.size()) {
            ...
            if (val[i + 1] == 't') {
              i++;
              if (std::get_if<std::shared_ptr<StructValue>>(
                      &args[current_arg])) {
                to_print += this->value_to_string(args[current_arg]);
              } else {
                this->errors.push_back(
                    {RTError("QC-B002: st formater requires a struct value.",
                             get_pos(target_val)),
                     "Error"});
              }
            } else {
              if (std::get_if<StringValue>(&args[current_arg])) {
                ...
                this->errors.push_back(
                    {RTError("QC-B002: qout(): s formater takes a string",
                             get_pos(target_val)),
                     "Error"});
              }
            }
          ...
          if (args.size() - 1 < current_arg) {
            this->errors.push_back(
                {RTError("QC-B001: too few arguments to qout().",
                         get_pos(target_val)),
                 "Error"});
          } else if (auto is_ftVal =
                         std::get_if<Number<float>>(&args[current_arg])) {
            std::stringstream ss;
            if (zero_pad)
              ss << std::setfill('0');
            if (width >= 0)
            ...
            this->errors.push_back(
                {RTError("QC-B002: qout(): f formater takes a float",
                         get_pos(target_val)),
                 "Error"});
          }
          break;
          ...
          if (args.size() - 1 < current_arg) {
            this->errors.push_back(
                {RTError("QC-B001: too few arguments to qout().",
                         get_pos(target_val)),
                 "Error"});
          } else if (auto is_dobVal =
                         std::get_if<Number<double>>(&args[current_arg])) {
            std::stringstream ss;
            if (zero_pad)
              ss << std::setfill('0');
            if (width >= 0)
            ...
            this->errors.push_back(
                {RTError("QC-B002: qout(): d formater takes a double",
                         get_pos(target_val)),
                 "Error"});
          }
          break;
        case 'c':
          ...
          if (i >= val.size()) {
            ...
            i++;
            if (std::get_if<std::shared_ptr<InstanceValue>>(
                    &args[current_arg])) {
              to_print += this->value_to_string(args[current_arg]);
            } else {
              this->errors.push_back(
                  {RTError(
                       "QC-B002: qout(): cs formater takes a class instance",
                       get_pos(target_val)),
                   "Error"});
            }
        ...
        case 'p':
          ...
          if (args.size() - 1 < current_arg)
            ...
            this->errors.push_back(
                {RTError("QC-B002: qout(): p formater takes a pointer",
                         get_pos(target_val)),
                 "Error"});
          }
          break;
        case 'x':
          ...
          if (auto is_intVal = std::get_if<Number<int>>(&args[current_arg])) {
            ...
            this->errors.push_back(
                {RTError("QC-B002: qout(): x formater takes a int or addr_t",
                         get_pos(target_val)),
                 "Error"});
          }
          break;
        case 'o':
          ...
          if (auto is_intVal = std::get_if<Number<int>>(&args[current_arg])) {
            ...
            this->errors.push_back(
                {RTError("QC-B002: qout(): o formater takes a int or address",
                         get_pos(target_val)),
                 "Error"});
          }
          break;
        case 'e':
          ...
          if (auto is_intVal = std::get_if<Number<int>>(&args[current_arg])) {
            ...
            to_print += ss.str();
          } else if (auto is_sVal =
                         std::get_if<Number<short>>(&args[current_arg])) {
            std::stringstream ss;
            if (zero_pad)
              ss << std::setfill('0');
            if (width >= 0)
            ...
            to_print += ss.str();
          } else if (auto is_llVal =
                         std::get_if<Number<long long>>(&args[current_arg])) {
            std::stringstream ss;
            if (zero_pad)
              ss << std::setfill('0');
            if (width >= 0)
            ...
            to_print += ss.str();
          } else if (auto is_ldVal =
                         std::get_if<Number<long double>>(&args[current_arg])) {
            std::stringstream ss;
            if (zero_pad)
              ss << std::setfill('0');
            if (width >= 0)
            ...
            this->errors.push_back(
                {RTError("QC-B002: qout(): e formatter takes a numeric type",
                         get_pos(target_val)),
                 "Error"});
          }
          break;
    ...
    if (func_name == "random") {
      ...
      if (args.size() == 0) {
        ...
        this->errors.push_back(
            {RTError("QC-B002: random(min, max) requires integer arguments",
                     get_pos(args[1])),
             "Error"});
      }
    ...
    if (func_name == "seed") {
      ...
      if (node->arg_nodes.size() != 1) {
        this->errors.push_back(
            {RTError("QC-B001: seed() requires exactly 1 argument", Position()),
             "Error"});
      }
    ...
    if (func_name == "to_qbool") {
      ...
      if (auto bv = std::get_if<BoolValue>(&cur_val)) {
        ...
        this->errors.push_back(
            {RTError("QC-B002: to_qbool() requires string argument to be qtrue "
                     "qfalse both or none.",
                     sv->pos),
             "Error"});
      } else {
        this->errors.push_back(
            {RTError("QC-B002: to_qbool() requires string or boolean argument",
                     get_pos(cur_val)),
             "Error"});
      }
    }
    ...
    if (func_name == "to_bool") {
      ...
      if (auto qbv = std::get_if<QBoolValue>(&cur_val)) {
        ...
        this->errors.push_back({RTError("QC-B002: to_bool() requires string "
                                        "argument to be true or false.",
                                        sv->pos),
                                "Error"});
      } else {
        this->errors.push_back({RTError("QC-B002: to_bool() requires string or "
    ...
    if (func_name == "to_int") {
      ...
      if (auto fv = std::get_if<Number<float>>(&cur_val)) {
        ...
        try {
          ...
          this->errors.push_back(
              {RTError("QC-B002: could not convert value to int.", sv->pos),
               "Error"});
        }
      } else if (auto qbv = std::get_if<QBoolValue>(&cur_val)) {
        int val = 0;
        if (qbv->valname == "none")
          val = 0;
        else if (qbv->valname == "qfalse")
          val = 1;
        else if (qbv->valname == "qtrue")
          val = 2;
        else if (qbv->valname == "both")
          val = 3;
        else
          throw RTError(
              "QC-WHAT: T̴̨̲̑̈̽́͘ḣ̸̢̛̜͈͖͉͖̲̫̹̖͖̐̓͂͑͒͑̀̆͛͌͘͝e̵̛̬̪̾̉̍̀͜ ̶͈̟̳̞̭͙̺͋͌̓̃́͗͝m̷̢̝̹͔̓̐̓̽̔͗͒̏̚ó̶͈̙̞̫͎͎͛̔̕̚͜ş̴̣̰̻̺͎̥̠̟̮̟̳͙̗͎̉̾̓̐̈̇̒̈͘͠ť̷̙̘̃́̿̀̀̔ ̷̟̦̪̖͉̯͓̭̮̤̺̼͋m̵͔̫͚͇̯̭̹̼͔̘̐́̈̅͛͛͒̄͑̿́̋̑̽̋̕͝ḛ̷̛̳̙̺̭̟͉̫̳͖̭̫̰̖͐̇̎̑͆͑̅́̇͌̈̿͜͜ŕ̷͚̬͓̤̰̳͙̣̣̾̋̊̅̈́͆̅̇͘̚͠͝c̷̡̫̪͐̉̽̊́̐̂̅͛͋̕ͅi̵̡̘̮̺̥̪̝͕͎̖̘͓̔̚f̶̧̡̧̧̤̩̪̙̤̩͎̜̤̺̍͑͊̏͐͜͠ų̵̪̖̘͎͓̺̼͔̰̬̬̆͛͊̒̿̓͑̀͛͛̽͠l̸̢̨͍̙͕͈͇̤̜̖̖̫̘̮̦̓̋̀̊̀̅̐̓̕͠ ̵̧͇̼̲͓̞̹̱͔͙̦̊̓̊̅͊͋̎̌̾͊̾͌̒͐̓t̵̯̯̮͙̯̪͉̰̱̬̊̈́͌̉̇͜ḫ̴̠̻̠̙̤͎̇̈̒͊̂̓̔̽͆̋͌̆ḯ̶̡̡̧̗̟̼̙ņ̸̩̠̜̙̫̩̩̻̟̀̽͒͆͛͌̈́͂̉̈́͐͘̕ͅg̷̲͛͋̐ ̵͕͈͎͓̝̝͍̐̅͌̉̽̿̓͜į̴̪̣̭̞̩̩̰͔̦̫͚̄̚̕ͅͅn̴̨͇̊́͌́͗́̈͊̄̕͘ ̴̨͎͍̘̤̼̞̥̥͎̪̑̐̈̏͑̑̍̒̎̉͐̈́̂̇͗t̶̢̡̞̜͇̩͍͕͚̦̖̜̭̗̲̭͒̈͑̿̍̄̽̉̽̽̐̃̇̈́̕̕͜ḩ̶̨͍̬̯̫̯̙̥̜͓̻̝̘̩̰͛̂̈́̃͝e̷͚̻͗͒̓̌̎͛̊͗͒̉͗̕͝ ̸̢̜̼̗͓͚͒̄̐̉̒͠ͅw̷̻͒͗̉̾̉̍̕͠ő̴̻̭͆̇̈́̈̉́̀̓́͑̊̆͊̕r̷̛̜̹̰̺̄͋͂͌̏̀̾́͑̑̇̒̕͝͝l̴̡̻̹̰͈̖͖̹̩͎͚̩͆͒͐͆̔̅̑̒̈́̀̍̓̉̓̉̏͘͜ḑ̷̼͕̱̠̳̖̥̓́̒̈́̔̅͌͌̓͒̀̿͂̕͜͝,̸̛̬̞̭̬̾͊̽̿͝ ̵̡̧̭̪̦̟̞̱̗̤̻͖̪̓͗̍͌̀̈́͐͠Î̶̢̬̲̘̲̭̮̇̂̃̌̓̌̄̔̋̄̎͆͝ ̶̩͉̦̣̖̪̼͍̲͋̊̋̕͠t̸̛̙̫͛́̓͂̎̓͋̇̎̅̇̚͠ͅh̵̡̼̝̦̳̹͇̼̯̟͖̖͙̝̀̈i̶̢̜͔͇̰̭̯͖̼̗͋̑̾̊̈́͒͊͗̇̂͜͜͝ͅṉ̸̞̤͆͐̉̐̎̔̌̌̕̚͠ķ̴̡̧̧̨̯͖̤̖̰̹̗̬̫͓̔͜,̸̮͇̙͙̐̋̉̃͑̎̒͘̕ ̶̧̢̛̹̟͇̬̼̫̻͔̫̭̳͈̲̓́̅̑̒͛̊̋̃̚͘̚͝͝ͅ ̶̧̛̬̙̤̿̽̾̐̏̂̍̀̕͜͠ï̴̮̱̘͇̜̓͋̕ͅs̴̻̯͓̲̝̘̥̪̖̻͙͋̌͒̋̀̄̄̚͘͝͝ ̵̻̞̭̯̬̩̲̫̲͔͈̖̇̈́́̀̏̈́̀̎̃̍̏̓̈͜͝ͅt̴̨̘̠̫̳̲̝̤̺̿̊̉̅͘h̴̛̩͇̬͔̯̥̬̙̳̞̙̉̄̏͛͂̀̓̈́͋́͗͜͠ͅe̴͉͈̺̗̭͆̊̎̑̎̑̂̊͛͒̉̐̈́͝͠͠ "
              "̸̩͖̤̻͔̻̲̤͙͎͒̾̂́͋̂̃̈̑͝į̷̳̠̖͈̥̲̎̔̌͂́͌̏̚̕͝ṉ̶̐̋̽ȁ̷͖̹̩̮̬̥̹̩̝̝͍̙̘̾̇̎͋̏͒̐̑̕͘b̸̢̧̨̯͈̤̲́̉i̴̡̨̧̢̳̼̞͙̺̭̮̹̺̪͖͎͠ͅl̶̨̮̫̻̻̱͔̔i̵̡̧̡͎̦̞̳̝̲̜̖̜̜̦͒͌͊̆͒̃̂̓̓̎̚t̸̢̢̞̫̺̱͈̦͍̜͚̦̫̰̬͂y̴͇͓̽ ̵̫̊͋ǫ̷̹̜̼̊͒͝f̷͉͚͕͕̲̗̻̲͛̉̌̈́̒̾͑̆̒͝ ̷̬̎̒̃t̷̢̳̹̙͉͙̖̪̏̾͊̾̓̂̑͌̈́͗͗̍͋͘̕̚h̴̨͔̠͇̖̠͇͖̻̩͔̹̜͂́̇ë̵̳̖̺́̋̈̊̕͘͠ ̵̛͍͚̩̇̆͆͊͐̉͠͠͠h̸̛̞̦̥̮́̐̈́̏͛̌͜ų̴̯̲̖͉͚͎͇͚̞̣͎̩̝̆̅͋̄̓͐̋́̌̄̿͐͛̈́́͝m̴͉̦̝̱̭̫̣̤̲̑̈̄̋̆̒̂̽̕͘͠ä̶̠̤͊͌̇̎̚n̵̢̛̝̮̮̻̠̫͓͚̻͎͆̓̈́̈͆̂̐̾̀̆̚͘ͅ ̶͍̰̥̳̅͊͆̆̅͒m̵̡̟͙̱̗͉͈̖̭̮͍̆͌̽̑͋̾̐̿̃́̄͒i̷̢̘̲̳͖̠̭͔̥͈̙̭̅̀͆̾̓͑̓̂̉̍́̚͠n̸̞̪̘̼̥̍̀̈͑̐́̐̈͐̾͂̈d̷̨̹͔͇͔̥̜̖̤͔̟̐͑͋͘͜ ̷̧̧̩̦̝̙̤̮̤̟͙̭̞̟̓͋͗̄̌͌́ͅţ̶͔̬͖͊̓̿ó̵̧̝̼͕͕̣͎̰̾͛̕ ̵͎͇̖̺͕̜͈̳̥̞̭̗̝̀c̵̭̭̺̜̆͛̕͜ŏ̸̢̗͕̂́͛̈̐̑̅͋́̈́̈́͘͠r̸̨̧͖̝̞̘̆̈̅͗̈́̂̂̍̀͊͑͆̀̓̄̍r̵̻̻̤̞̭̖̮͓̝̥̬̼̦͙̙͑̍̎͋͛̓̐͒̅̔͜ȩ̶̼̲̱̦̞̞̱̞͎̟̱̤̱͝ľ̵̗͈̮͔̭͔̼̻̓̄̓͋͂̌̓̓̎̉̈́̅͑̈́͘͜͝ͅa̵̛͓̪̫͌͘ț̴̛̳͚̖̟̖̱͒͗͐͛́̋̏̋̒͆̃̓͘ȩ̴͙̱͉̮̫̭̠̭̠͚̲̱̱̮̀̓͋ ̷̢̥̭̰̻͓̝̱̾̓̍͆͐̌͐̽̆͛͐̿̽̉̚ͅͅͅa̵̫̬͖̳̳̞̾̈́́̈̈́͗̎̆̾̀̎̀̾͘̕͜͝͝ͅḻ̸̡͔͖͇̟͉̦͉̤̣̟̩̪̺̅̉̄̾̍͒̕ͅl̸̙͕̼̘̩̳͎͙̤̗̐ ̷̜̲͌͜ơ̵̧͕̪̥̬͓̞͔͆̀͐̄̿͠ḟ̸̜̖̭̑̉̈́͂͌̍͐́̊̋͋̅̐͝͝ ̵̛̣̞̥̳͇͔͕̯̩̝̞͛̊̓̇́̐͐͆i̵̡̧͉̮͔̗̖̞͉̬̅͛̉̔̓̎̚ţ̷̰͔͖̬̝͍̪͐͆͒̾̇̑̿̿̾͘ş̸̣̗̰̗̬̂͋̚͘̚ ̵̧̡̬͕̙͇͔̙̠͎̞̫̺͎̌̂̀́̋̅͑͋͐͜͝͠c̴̡͍͎̘͖̰͍̥̪̬̝͉̒ͅơ̷̢̨͍͉̲̝̻͚̼̗̫̺̆̃̍̓̀̎̇̏̀͊͒̓̔͗͝͝n̴̢̰͙̪͙̲͍͐́͂̀̍́̈́͑̎̕̕͠ţ̵̛̘̬̺̥̣͍̦̣͖̼̗̳̭́̾̃͐̓̓̑̃̏̓̚̕ͅȇ̶͓̟̘̭̦̝̹̭̝͍͕̏̉̀̿n̷̘̲͎̙̰̐͜t̸̢̢̧̟̟͙̠͚̬̤͚̠̞̣̹̜̎s̶̢͍͕̲̺̰̲̳̒̊̊̆̎͗̈̍͝͝͠.̴̡̢̨̞̼̥͉̥̱̠̩͔͗͑̀̇̑̓̅̂͊̚͜͝ "
              "̵̡̧̳͇͚̙̖̼̯͙̟̾̓̔͑̐́̚͝\nW̸̢̬̹̟͑̽͒̎͌̚͘ę̵͎̹̻̣͕̺͍̞̫̟̃̈́͒̉́̚̚͜͜͠͠͝ͅ ̵̭̖̮̦́̀͝͠l̴̡̛̟͇͓͚̘̄̒̓͠i̶̡̡̤̰̹̫̱̬͚̘̍͛̈́͑̓̕̚͝v̴̢̡̤͑͒͌̏̌̀̓̄͆̾̔͒̐͝͠͠ȇ̷̫̈̾̓̈̍͂͛̚ ̸̻̖̜̣̬̱̱̔͛͗̚͠ơ̶̮̥̈̄͋͆̿̋̀̏̍̇͊͘̚n̴̛̘̩͔̤̮͈̻͖̞̙̙̺̓̊̍̉̓̑̾̚ ̶̝̱͚̠̦̹̐̾̌͠à̶̞̦̖͇͙̟̳̥͊͊̾̽ ̸̡̛̛͈̦̖̻̭͍͖̩̋́̇̒̾̏̆̕͝p̶̧͈̳̪͈̘̣̺̙͔̥͓̠̠͇̩͋̈́̑̕͜l̶̡̹̱͓͖͇̮͌͆̇͐͐͂̓̑ǎ̴͚̞̫͎̬̙c̴̞̖̗̍̈́̕͝i̴͎̐̀̈́̑̀ḑ̸͉̜̬̼̼̬̃̈̈̍̔̌͒͌̌͝ͅ ̶̗́͆̽͆̀͛i̵̛͔͕̫͎͔̰̯̬̞͈s̸̭̳͍̖͓̼̩̺̘̖̗̀̾̈͂̈́l̵̡̙̫̣͉̱̼͖̔͑̃͌̿̍̔̊̈́́̈͘͝͠a̸̡̛͈̲͖̒̿̍͌͊̂̔̊͐̐̽͑͠n̵̨̡͇̩̱̤͉̭̖͓̗̤͎̦̍͆́̋̆̈́̽͒̔̀̂̓͛̂͜d̷̡͕̱͙͔̲͔̲̟̍̀͜͜ ̴͍͈̝̺̈̃̉͑̅͆ǫ̷̧̢̛͇̼̪͓̦͙͔͆̍f̵̡̲̠̲̤͖͙̭̫͎̈́ ̵̤̥͇̪̺̙̞̰̣̩̝͖̓̔̈́̈́̔̉̄̑̇̕ͅi̷̢̖̩̣̪̝͇͚̣͖̟͔̯͈̼͙̬̔̒ģ̴̨̛̦͕̼̦̹̻̜̜̃̍̇̈̉͝ͅͅͅň̸̢̢̛̯̟̹̤̘̬̟̝͉͆̀͊̄̾́͒̔̋͛̏̕͝͝ô̵͖̩̳͕͍̜̱͓̙̜̎r̷̺̟̊͌̆̆̂̈́̌̍͐̒̓̈́̐̄̕ả̷̛͙̙̥̣̜̘̖́̈̾̉̾̈́́͠n̸͉̩̺̣̙̳̣̲͙͖̋͋͆̍̅̇͆͐̌̏̾͜c̸͕̥̩̱̱͖̮͈̲͑̀̋͐̍̆̅͆e̵̯̖͔̓̄̓͆̎̆̒̀̑͝͝ ̶̦͖͒͌̌͋͘i̸̳͓̎̐n̸̝͊ ̷̨̧͈̮̲̘̪̉̐̀̈́́t̴̡̗̖̠̘͈͍̞̲̘́̓̈́̔̐͗̀͆͝ͅh̵̤̹̞̞͖̣̼̀́͐̔̏͘͠ͅe̶͍̫̫͔͊̑͝ ̷̧̥̥͙͒͗̐͘͠m̸̨̢̛͖̦͇̗͍̥͍̫̗͉̿̉̄͆͑̏͆̈́̿̊͘͠͠i̴̧͚͎̼̹͖̰̠̰̰̺̯̯͛̎͛́͑͑̈͛͋͆̍̾̈̂͜͜͠ͅd̴͙̹̝̣͇͐̑́̏͌̀͋̀̅͐̽̈̍́͊̚͝ͅs̸̢̢̥̜͖̻̺̪̹̝̫̈ẗ̶̡̤͓͇̙̯̟̤̻̜͈͙̼̦̭̥̟́̉̓́̉̉̑̑̊̇͋̉͌̓̚͝ ̵̠̥̆̐̔̅͂̂́̓̊̇͝͝ò̵͇͊̓͐͂͂̀͛f̴̨̹͍͎͒̐̃̈́̋̅̀͝ ̵̲̣̰̪̹̘̤̭̙͗̒b̴̲̙͕͖̦̙͍̪͍̉͒̔͗̎͂̈́́͊̌̅͆̍̂̕ļ̶̥͖̱̜͈̟͉̐͘a̴̟̹̫͂̊̎̄͐̂͠c̸̢͍͈͔̮̹͇̼̙̟͓̮̭̩̑͂́̏̍͛̿͋̎̌̌̕k̶̬̝̰̩͕̈̈̓̌̔̈̿́͒̍̔̐̅̓͘͝ "
              "̸̢̢̡̛̟͍̤̥͙̝̘͍͇͓͔̣̭̜̊͊̎͂͆̓̃̎̿̕̕͘͝͝s̷̢͖͙̥͉̘̄̑͗̉̒̅́͋̏̾̆̅̔́̈́̕͝ͅͅê̶͇͉͈̰̭̼͎̔̿́̈́͆́͆́͘͘ͅā̸̜̠͇́̍̈͂͐͐͂̑̽͘͝s̵̢̳̆̆̑̉̔̑̊͌̂ͅ ̸͚̬͒̈́͛͗́̃̀͛̈́͆̃̀̂̀̕̚͘ố̸̡̡̹̼̙̤̘̱̪̻̒̿̿̈̂f̴̛̛̭̹̘́̐͐̈́̌͐̒̏͌̍͑͘͝ ̷̼̮̙̜̈̎͛̓͐͋̾͗̀́̂̇̇̅͝ͅī̴̤̥͇̲̣̙̦̩̜̼͈̲͓̼͔̌̏͗̀̀͐̕͠n̷̡̹̘̞̦̙̙̦̮̙͎͆͗̒̎̂̊̈͑̏̈́̑͋̃͋́̇̚ḟ̵̧̧͚̤͚̫͍̰̗͙͚̜̲̹̗̺͒̏͆͛͌̒̊͠i̷̢̛̛̲̥͚̳͚̟̼̲̤̦͕̻̪̐͊̄̈́͘̕͜n̵͎̘̳̘̟̕i̸̜̮̘̠̘̘̹̰̓̿̍̅̓̈́̊͗̇̊̚͝͝t̸̛̩͇̲̪̟̭̱̝̯͍̘̻̘͆͛̊̋̎̎̍̈́͊͊̽̊̽̊̚͠y̷̨̡̛̳͓͌͛͐̽̈́,̸̡̡̬͙̩͍̜̭͓̦͉̦̀̈́͐̏͊̌̅̀͑͘͘ͅ ̵̞̠̜͉̣̃̌͌̒͗̈̓̂̿͠a̶̭͚͚͓̳̼̎͒́̍̅͆̆̅̕n̵̹͌̿̆̎͐̒͂͆͛͆͒̍͊͗̕͠ḑ̴̺̟̳͎̩͉̮̈́̄ ̸͍̾̈ȉ̸̡̧̦̰͈͓͎̦̳̈̄̋̅̓͗̀̍͝t̵̰̊͌̓̅͠ ̵̦̰̰̬̏̌̅̀͆̑̔̀͒̿̚͠͝ẁ̸̱̄͋͛͆̊̕a̵̢̧̝͚͕̫̺͙̫̹̫͖̳͆́̓̈́͒͛͘̕͝s̵̨̗͔̦̻̘̙͕͓̠̓̈́̓̎͂͆̄̀̀̕ ̶̛̦̅̈́̈̋̆͆̄̆͑͝n̴̨͈̦̙̝͖̦͌̔̎̒̿͊̑́̽͋̈́̅̄̎͌̕͝ȏ̸̹͇̪͙̜̄̊̍̒̉͛̂͘͠ť̴͈́͛̇̿̀̄̈͛̾̈̀̀̏̕ ̸̢̢͍͍͉̫̰̱͎̣̲̻͆́m̷̢̝͖̩̖͉̹̯̂̅̓́̍̍̍̒̀̓̚͝e̷̛̻͎͕̙̰͎̟͚̜̮͐̑̄̚ả̴̢̨͉̮̫̩̹͖̭̞̓̈n̶̡̩͇̦͚͔͈̩̞̫͕̤̘̎ṯ̶̢̟͉͕̳͔̮̫͚̤̹̹̥̦͉͕͋͛̎͊́͋͛̚ ̸̛̛̛͇̲̪̲̬̼̝̋̄̈̑͛͐̎͒̒̾̉̑͘͜͠t̴̥͍̊͑̀̀́̉́̍̀̓́̾̓̕͘͘͠h̴̢̡͚̞̖̙͇̣͖̺̙̣̤͖͆͊͊̊͌͂̆́̉͌͐͘͠͝a̶̛̛͎̖̥̍͑͆̾̄͑̿̄͆̑́̕t̷̹̬̩̺̠͕̜̗̲̓̃̒̿͒ ̶̨̢͓̺̺̞̺̺͙̟̬͙̯̻̝͗͒̊̋͂́̕͠ẅ̷̛̳̠̤̲̻́ę̵̦̫̺̞͖͔̭̺̐͒̋̓͆̔́͑̿͛̈͊̿̿̀̕͜͠ ̵̡͚͇̗̞̺̗͎̪͚͋̋̽̈́̾̆͋̆̅͑̚͝s̷͓͈̰̹͓͕̖̜̞̼͙͕͚̬̖̦̩̓̌̿h̴͔̱͈̲̳̤̠͙̗̙̊̑͂͑̇̽̂̋̕̕͝o̵͙̞͚̺̅̃̈́͑̍͊̕͠ư̸͉̻̮̲͇͛̀̎͌l̷̠̇̋̀̕ͅd̴̻͕̑̔̌̾͗͗͒͌̒ ̵̺̹̥̞̯̠͕̘̳̪̄́̅͊̔͛́̀̈́̕͠͠v̵̧̀̆̊̄̊̌͠ỡ̴̹̒́̂͑̌̄̊͝͝ỳ̷̧͈͓̻̥̩̳̫̦̖̼͑̑̈́̉̀͌̈̄͘͠a̷̤̚g̶̡̨̣̗̣̰̬͈̗͖̩̖̭̋̆e̷̖̘͈͍̮̰͙͈͕̫̙͔͕̖̿́̋͒̒͊̌̚͠ "
        ...
        this->errors.push_back(
            {RTError("QC-B002: to_int() cannot take that argument type.",
                     get_pos(cur_val)),
             "Error"});
      }
    }
    ...
    if (func_name == "to_float") {
      ...
      if (auto fv = std::get_if<Number<float>>(&cur_val)) {
        ...
        try {
          ...
          this->errors.push_back(
              {RTError("QC-B002: could not convert value to float.", sv->pos),
               "Error"});
        }
      } else {
        this->errors.push_back(
            {RTError("QC-B002: to_float() cannot take that argument type.",
                     get_pos(cur_val)),
             "Error"});
      }
    }
    ...
    if (func_name == "to_double") {
      ...
      if (auto fv = std::get_if<Number<float>>(&cur_val)) {
        ...
        try {
          ...
          this->errors.push_back(
              {RTError("QC-B002: could not convert value to double.", sv->pos),
               "Error"});
        }
      } else {
        this->errors.push_back(
            {RTError("QC-B002: to_double() cannot take that argument type.",
                     get_pos(cur_val)),
             "Error"});
      }
    }
    ...
    if (func_name == "to_char") {
      ...
      if (auto sv = std::get_if<StringValue>(&cur_val)) {
        ...
        this->errors.push_back(
            {RTError("QC-B002: to_char() cannot take empty string.", sv->pos),
             "Error"});
      } else if (auto cv = std::get_if<CharValue>(&cur_val)) {
        return *cv;
      } else {
        this->errors.push_back(
            {RTError("QC-B002: to_char() cannot take that argument type.",
                     get_pos(cur_val)),
             "Error"});
      }
    }
    if (func_name == "to_string") {
      ...
      if (auto fv = std::get_if<Number<float>>(&cur_val)) {
        ...
        this->errors.push_back(
            {RTError("QC-B002: to_string() cannot take that argument type.",
                     get_pos(cur_val)),
             "Error"});
      }
    }
    if (func_name == "split") {
      ...
      if (!std::holds_alternative<StringValue>(str_val) ||
    ...
    if (func_name == "len") {
      if (node->arg_nodes.size() != 1) {
        this->errors.push_back(
            {RTError("QC-B001: len() requires exactly 1 argument", Position()),
             "Error"});
      }
      ...
      if (auto sv = std::get_if<StringValue>(&val)) {
        ...
        this->errors.push_back(
            {RTError("QC-B002: len() requires string argument", get_pos(val)),
             "Error"});
      }
    }
    ...
    if (func_name == "fopen") {
      if (node->arg_nodes.size() != 2) {
        this->errors.push_back(
            {RTError(
                 "QC-B001: fopen() requires exactly 2 arguments (path, mode)",
                 Position()),
             "Error"});
        return VoidValue();
      }
      ...
      if (!path_str || !mode_str) {
        this->errors.push_back(
            {RTError("QC-B002: fopen() requires string arguments", Position()),
             "Error"});
        return VoidValue();
      }
    ...
    if (func_name == "fwrite") {
      if (node->arg_nodes.size() != 2) {
        this->errors.push_back(
            {RTError(
                 "QC-B001: fwrite() requires exactly 2 arguments (file, data)",
                 Position()),
             "Error"});
        return VoidValue();
      }
    ...
    if (func_name == "substring") {
      if (node->arg_nodes.size() != 3) {
        this->errors.push_back(
            {RTError("QC-B001: substring() requires 3 arguments", Position()),
             "Error"});
      }
    ...
    if (func_name == "join") {
      if (node->arg_nodes.size() != 2) {
        this->errors.push_back(
            {RTError("QC-B001: join() requires 2 arguments", Position()),
             "Error"});
      }
        ...
        if (auto sv = std::get_if<StringValue>(&delim_val)) {
          ...
          for (size_t i = 0; i < (*lv)->elements.size(); i++) {
            ...
            if (auto s = std::get_if<StringValue>(&el)) {
              ...
              this->errors.push_back(
                  {RTError("QC-B002: join() requires list of strings",
                           get_pos(el)),
                   "Error"});
            }
          }
      ...
      this->errors.push_back({RTError("QC-B002: join() requires (list, string)",
                                      get_pos(list_val)),
                              "Error"});
    }
    if (func_name == "contains") {
      if (node->arg_nodes.size() != 2) {
        this->errors.push_back(
            {RTError("QC-B001: contains() requires 2 arguments", Position()),
             "Error"});
      }
    ...
    if (func_name == "startswith") {
      if (node->arg_nodes.size() != 2) {
        this->errors.push_back(
            {RTError("QC-B001: startswith() requires 2 arguments", Position()),
             "Error"});
      }
    ...
    if (func_name == "endswith") {
      if (node->arg_nodes.size() != 2) {
        this->errors.push_back(
            {RTError("QC-B001: endswith() requires 2 arguments", Position()),
             "Error"});
      }
    ...
    if (func_name == "trim") {
      if (node->arg_nodes.size() != 1) {
        this->errors.push_back(
            {RTError("QC-B001: trim() requires exactly 1 argument", Position()),
             "Error"});
      }
      ...
      this->errors.push_back(
          {RTError("QC-B002: trim() requires string argument", get_pos(val)),
           "Error"});
    }
    if (func_name == "replace") {
      if (node->arg_nodes.size() != 3) {
        this->errors.push_back(
            {RTError("QC-B001: replace() requires 3 arguments", Position()),
             "Error"});
      }
    ...
    if (func_name == "malloc") {
      this->errors.push_back(
          {RTError(
               "Manual Memory Management is only fully supported in QuantumC's "
               "compiler, and thus, will be worse in the interpreter, such as "
               "malloc in interpreter taking a string, not a number.",
               Position()),
           "warning"});
      if (node->arg_nodes.size() != 1) {
        throw RTError(
            "QC-B001: malloc expects exactly 1 argument (type name string)",
            Position());
      }
      ...
      if (sz == 0) {
        throw RTError("QC-B002: Unknown or unsupported type in malloc: " +
                          type_name,
                      get_pos(type_val));
      }
      ...
      return PointerValue::heap_ptr(type_name, heap_id)
          .set_pos(get_pos(type_val));
    }
    if (func_name == "free") {
      this->errors.push_back(
          {RTError(
           ...
           "warning"});
      if (node->arg_nodes.size() != 1) {
        throw RTError("QC-B001: free expects exactly 1 argument (pointer)",
                      Position("", "", 0, 0, 0));
      }
    ...
    if (func_name == "calloc") {
      this->errors.push_back(
          {RTError(
           ...
           "warning"});
      if (node->arg_nodes.size() != 2) {
        throw RTError("QC-B001: calloc expects 2 arguments (count, type name)",
                      Position());
      }
      ...
      size_t sz = this->get_sizeof_type(type_name);
      if (sz == 0) {
        throw RTError("QC-B002: Unknown or unsupported type in calloc: " +
                          type_name,
                      get_pos(type_val));
      }
    ...
    if (func_name == "realloc") {
      this->errors.push_back(
          {RTError(
           ...
           "warning"});
      if (node->arg_nodes.size() != 2) {
        throw RTError("QC-B001: realloc expects 2 arguments (ptr, new_size)",
                      Position());
      }
      ...
      auto ptr = std::get_if<PointerValue>(&ptr_val);
      if (!ptr) {
        throw RTError("QC-B002: realloc expects a pointer as first argument",
                      get_pos(ptr_val));
      }
      ...
      if (ptr->is_null) {
        ...
        size_t heap_id = context->heap_alloc(def_val);
        return PointerValue::heap_ptr("void", heap_id)
            .set_pos(get_pos(ptr_val));
      }
      ...
      if (std::holds_alternative<std::shared_ptr<ArrayValue>>(block)) {
        throw RTError("QC-B002: realloc not allowed on array blocks (use "
                      "calloc for arrays)",
                      ptr->pos);
      }
      ...
      if (new_size == 0) {
        context->heap_free(ptr->heap_id);
        return PointerValue::heap_ptr(ptr->pointee_type, 0)
            .set_pos(get_pos(ptr_val));
      }
      return *ptr;
    }
    if (func_name == "ternary" && !node->arg_nodes.empty()) {
      ...
      if (this->context->get_type_name(is_tr) !=
          this->context->get_type_name(is_fl)) {
        throw RTError("QC-B002: ternary arg 2 type must equal arg 3 type",
                      varacc->var_name_tok.pos);
      }
    ...
    if (ut_it != context->user_types.end() &&
      ...
      if (info.is_abstract_class) {
        errors.push_back(
            {RTError("QC-B002: Cannot construct abstract class", Position()),
             "Error"});
        return VoidValue();
      }
      try {
        ...
        if (child_ut != context->user_types.end() &&
          ...
          for (auto &arg : node->arg_nodes) {
            if (auto spread = std::get_if<std::unique_ptr<SpreadNode>>(&arg)) {
              ...
              if (auto varacc_ptr =
                      std::get_if<std::unique_ptr<VarAccessNode>>(&arg)) {
          ...
          if (final_args.size() > ctor->params.size()) {
            context->pop_scope();
            this->errors.push_back(
                {RTError("QC-C002: Too many arguments to parent constructor '" +
                             func_name + "'",
                         Position()),
                 "Error"});
            return VoidValue();
          ...
          for (size_t i = 0; i < ctor->params.size(); ++i) {
            ...
            if (i < final_args.size()) {
              ...
              context->pop_scope();
              this->errors.push_back(
                  {RTError("QC-C004: Missing argument to parent constructor '" +
                               func_name + "'",
                           get_pos(value)),
                   "Error"});
              return VoidValue();
  ...
  if (!std::holds_alternative<FunctionValue>(target_val))
    this->errors.push_back(
        {RTError("Can only call functions", Position()), "Error"});

  FunctionValue fval = std::get<FunctionValue>(target_val);
  if (!fval.func)
    this->errors.push_back(
        {RTError("Invalid function value", Position()), "Error"});
  ...
  try {
    if (final_args.size() > func->params.size()) {
      ...
      this->errors.push_back(
          {RTError("QC-C001: Too many arguments", Position()), "Error"});
    }
    for (size_t i = 0; i < func->params.size(); i++) {
      ...
      if (i < final_args.size()) {
        ...
        this->errors.push_back(
            {RTError("QC-C003: Missing argument", Position()), "Error"});
        continue;
      }

      std::string expected_type = it_param->type.value;
      bool is_fn_type =
          (expected_type == "fn" && it_param->signature.has_value());
      ...
      if (is_ref_param && !lvalue_name.empty()) {
        ...
        if (norm(base_expected) != norm(actual_type)) {
          this->errors.push_back(
              {RTError("QC-R003: Cannot bind ref parameter '" +
                           it_param->name.value + "' of type " + base_expected +
                           "& to '" + lvalue_name + "' of type " + actual_type,
                       it_param->name.pos),
               "Error"});
          continue;
        }
        ...
        context->define_reference(it_param->name.value, lvalue_name,
                                  base_expected + "&", it_param->name.pos);
        continue;
      }
      ...
      if (is_fn_type) {
        ...
        if (expected_type == actual_type) {
          ...
          if (actual_type == "long int")
            actual_type = "int";
          if (actual_type == "short int")
            actual_type = "int";
          if (actual_type == "long double")
            actual_type = "double";
          if (expected_type == "long int")
            expected_type = "int";
          if (expected_type == "short int")
            expected_type = "int";
          ...
          if (expected_type == "list<auto>") {
            if (actual_type.find("list<") != std::string::npos ||
              ...
              if (auto av = std::get_if<std::shared_ptr<ArrayValue>>(&value)) {
                value = std::make_shared<ListValue>(strip((*av)->element_type),
                                                    (*av)->elements);
              }
            }
            ...
            if (strip(expected_type) == inner && expected_type != inner) {
              ...
              if (auto lv = std::get_if<std::shared_ptr<ListValue>>(&value)) {
                value = std::make_shared<ArrayValue>(strip((*lv)->element_type),
                                                     (*lv)->elements);
              }
            }
          } else {
        ...
        if (!types_compatible) {
          ...
          this->errors.push_back(
              {RTError("Argument type mismatch: expected " + expected_type +
                           ", got " + actual_type,
                       Position()),
               "Error"});
        }
    ...
    if (!func->return_types.empty()) {
      ...
      if (ut_it != context->user_types.end() &&
        ...
        if (auto arr_ptr =
                std::get_if<std::shared_ptr<ArrayValue>>(&re.value)) {
          NumberVariant converted =
              convert_array_to_struct(*arr_ptr, expected_type, context);
...
NumberVariant Interpreter::operator()(NumberNode &node) {
  if (node.tok.type == TokenType::INT) {
    try {
      ...
      this->errors.push_back(
          {RTError("Integer out of range", node.tok.pos), "Severe"});
    }
  }
...
NumberVariant Interpreter::operator()(std::unique_ptr<VarAccessNode> &node) {
  ...
  if (!context) {
    this->errors.push_back(
        {RTError("Context not initialized", node->var_name_tok.pos), "Severe"});
  }
...
NumberVariant Interpreter::call_instance_method(
  ...
  if (method->access == "protected" &&
      !in_class_or_derived_context(className)) {
    this->errors.push_back(
        {RTError("Method '" + mname + "' of class '" + className +
  ...
  if (args.size() > method->params.size()) {
    context->pop_scope();
    this->errors.push_back(
        {RTError("QC-C002: Too many arguments to method '" + mname + "'", pos),
         "Error"});
    return VoidValue();
  }
...
NumberVariant Interpreter::operator()(std::unique_ptr<BinOpNode> &node) {
  ...
  if (node->op_tok.type == TokenType::RSHIFT) {
    ...
    if (auto left_binop =
            std::get_if<std::unique_ptr<BinOpNode>>(&node->left_node)) {
    ...
    if (is_qin_start || is_qin_chain) {
      ...
      if (auto var =
              std::get_if<std::unique_ptr<VarAccessNode>>(&node->right_node)) {
        ...
        if (type == "int" || type == "short int" || type == "long int" ||
          ...
          try {
            ...
            this->errors.push_back(
                {RTError("Cannot parse integer", node->op_tok.pos), "Error"});
          }
        } else if (type == "float") {
          try {
            ...
            this->errors.push_back(
                {RTError("Cannot parse float", node->op_tok.pos), "Error"});
          }
          ...
          try {
            ...
            this->errors.push_back(
                {RTError("Cannot parse double", node->op_tok.pos), "Error"});
          }
        } else if (type == "string") {
          ...
          this->errors.push_back(
              {RTError("Cannot read input into type " + type, node->op_tok.pos),
               "Error"});
        }
      ...
      auto to_size_t = [](const NumberVariant &v) -> std::optional<size_t> {
        ...
        if (auto l = std::get_if<Number<long long>>(&v))
          return static_cast<size_t>(l->value);
        if (auto s = std::get_if<Number<short>>(&v))
          return static_cast<size_t>(s->value);
        if (auto u = std::get_if<Number<uintptr_t>>(&v))
          return static_cast<size_t>(u->value);
        return std::nullopt;
      };
      ...
      if (!shift_amount) {
        this->errors.push_back(
            {RTError("Shift amount must be an integer", node->op_tok.pos)});
        return VoidValue();
      }
    ...
    return std::visit(
        [&, this](auto &&arg) -> NumberVariant {
          ...
          if constexpr (std::is_same_v<T, Number<int>> ||
            ...
            return Number<decltype(arg.value)>{
                static_cast<decltype(arg.value)>(result)};
          }
          this->errors.push_back(
              {RTError("Cannot shift non-ints", node->op_tok.pos)});
          return VoidValue();
        },
        left);
  }
  if (node->op_tok.type == TokenType::L_ROT) {
    ...
    return std::visit(
        [&, this](auto &&arg) -> NumberVariant {
          ...
          this->errors.push_back(
              {RTError("Cannot rotate non-ints", node->op_tok.pos)});
          return VoidValue();
        },
        left);
  }
  ...
  if (node->op_tok.type == TokenType::PIPE ||
      node->op_tok.type == TokenType::AMPERSAND ||
      node->op_tok.type == TokenType::BITWISE_XOR) {
    ...
    auto perform_bit_op = [&](auto a, auto b, TokenType op) -> NumberVariant {
      switch (op) {
      case TokenType::AMPERSAND:
        return Number<decltype(a)>{static_cast<decltype(a)>(a & b)};
      case TokenType::PIPE:
        return Number<decltype(a)>{static_cast<decltype(a)>(a | b)};
      case TokenType::BITWISE_XOR:
        return Number<decltype(a)>{static_cast<decltype(a)>(a ^ b)};
    ...
    if (auto lhs = std::get_if<Number<int>>(&left)) {
      if (auto rhs = std::get_if<Number<int>>(&right))
        return perform_bit_op(lhs->value, rhs->value, node->op_tok.type);
  ...
  if (node->op_tok.type == TokenType::AND) {
    ...
    if (auto inst_ptr = std::get_if<std::shared_ptr<InstanceValue>>(&left)) {
      ...
      ClassMethodInfo *method =
          find_method_with_args((*inst_ptr)->class_name, "operator&&", args);
      if (method) {
  ...
  return std::visit(
      [this, &node, right, left](const auto &L,
        ...
        if (node->is_f) {
          ...
          return std::move(
              StringValue(l_str + r_str).set_pos(node->op_tok.pos));
        }

        if (node->op_tok.type == TokenType::EQ_TO) {
          try {
            return BoolValue(
                values_equal(left, right, node->op_tok.pos) ? "true" : "false");
          } catch (RTError &e) {
            return BoolValue("false");
          }
        }
        ...
        if constexpr (std::is_same_v<T1, StringValue> &&
                      std::is_same_v<T2, StringValue>) {
          if (node->op_tok.type == TokenType::PLUS) {
            return std::move(
                StringValue(L.value + R.value).set_pos(node->op_tok.pos));
          }
          this->errors.push_back(
              {RTError("QC-T005: Only '+' and logical expresions are supported "
                       "for strings",
                       node->op_tok.pos),
               "Error"});
          ...
          this->errors.push_back(
              {RTError(
                   "QC-T006: Cannot perform operations on string and number",
                   node->op_tok.pos),
               "Error"});
          ...
          this->errors.push_back(
              {RTError(
                   "QC-T006: Cannot perform arithmetic operations on a Boolean",
                   node->op_tok.pos),
               "Error"});
        } else if constexpr (std::is_same_v<T1, QBoolValue> ||
                             std::is_same_v<T2, QBoolValue>) {
          this->errors.push_back({RTError("QC-T006: Cannot perform arithmetic "
          ...
          this->errors.push_back(
              {RTError(
                   "QC-T006: Cannot perform arithmetic operations on nothing",
                   node->op_tok.pos),
               "Error"});
        } else if constexpr (std::is_same_v<T1, std::shared_ptr<ArrayValue>> ||
                             std::is_same_v<T2, std::shared_ptr<ArrayValue>>) {
          this->errors.push_back(
              {RTError(
                   "QC-T006: Cannot perform arithmetic operations on arrays",
                   node->op_tok.pos),
               "Error"});
        } else if constexpr (std::is_same_v<T1, std::shared_ptr<ListValue>> ||
                             std::is_same_v<T2, std::shared_ptr<ListValue>>) {
          this->errors.push_back(
              {RTError("QC-T006: Cannot perform arithmetic operations on lists",
                       node->op_tok.pos),
               "Error"});
        } else if constexpr (std::is_same_v<T1, std::shared_ptr<MapValue>> ||
                             std::is_same_v<T2, std::shared_ptr<MapValue>>) {
          this->errors.push_back({RTError("QC-T006: Cannot perform arithmetic "
                                          "operations on a map/dict",
                                          node->op_tok.pos),
                                  "Error"});
        } else if constexpr (std::is_same_v<T1, std::shared_ptr<StructValue>> ||
                             std::is_same_v<T2, std::shared_ptr<StructValue>>) {
          this->errors.push_back(
              {RTError(
                   "QC-T006: Cannot perform arithmetic operations on a Struct",
                   node->op_tok.pos),
               "Error"});
          ...
          this->errors.push_back(
              {RTError(
                   "QC-T006: Cannot perform arithmetic on multi-return values",
                   node->op_tok.pos),
               "Error"});
  ...
  return std::visit(
      [&](auto const &L, auto const &R) -> NumberVariant {
        ...
        if constexpr (std::is_same_v<TL, Number<short>> ||
          ...
          if constexpr (std::is_same_v<TR, Number<short>> ||
            ...
            throw RTError(
                "QC-T002: Right side of compound assignment must be numeric",
                pos);
          }
        } else {
          throw RTError(
              "QC-T002: Left side of compound assignment must be numeric", pos);
        }
      },
      left, right);
}
NumberVariant Interpreter::operator()(std::unique_ptr<AssignExprNode> &node) {
  ...
  if (auto var = std::get_if<std::unique_ptr<VarAccessNode>>(&target)) {
    ...
    if (op == TokenType::EQ) {
      ...
      TokenType binop_type;
      if (op == TokenType::PLUS_EQ)
        binop_type = TokenType::PLUS;
      else if (op == TokenType::MINUS_EQ)
        binop_type = TokenType::MINUS;
      else if (op == TokenType::MUL_EQ)
        binop_type = TokenType::MUL;
      else if (op == TokenType::DIV_EQ)
        binop_type = TokenType::DIV;
      else if (op == TokenType::MOD_EQ)
        binop_type = TokenType::MOD;
      else
        throw RTError("Unsupported assignment operator", pos);
  ...
  if (auto un = std::get_if<std::unique_ptr<UnaryOpNode>>(&target)) {
    if ((*un)->op_tok.type == TokenType::MUL) {
      ...
      if (pv->is_heap) {
        ...
        if (op == TokenType::EQ) {
          ...
          if (op == TokenType::PLUS_EQ)
            ...
            throw RTError("Unsupported assignment operator for heap pointer",
                          (*un)->op_tok.pos);
          }
      ...
      if (sym.is_const) {
        throw RTError("QC-T001: Cannot assign through const pointer target '" +
                          pv->symbol_key + "'",
                      pv->pos);
      }
      ...
      if (op == TokenType::EQ) {
        ...
        if (op == TokenType::PLUS_EQ)
          ...
          throw RTError("Unsupported assignment operator for *pointer",
                        (*un)->op_tok.pos);
        }
...
NumberVariant Interpreter::operator()(std::unique_ptr<UnaryOpNode> &node) {
  ...
  if (node->op_tok.type == TokenType::QNOT) {
    ...
    if (auto inst_ptr = std::get_if<std::shared_ptr<InstanceValue>>(&val)) {
      ...
      if (!mname.empty()) {
        ...
        if (method) {
          NumberVariant result = call_instance_method(
              *inst_ptr, method, std::vector<NumberVariant>{},
              node->op_tok.pos);
          return result;
        }
      }
    ...
    return std::visit(
        [&](auto &&arg) -> NumberVariant {
          ...
          if constexpr (std::is_same_v<T, Number<int>> ||
                        ...
                        std::is_same_v<T, Number<short>>) {
            return Number<decltype(arg.value)>{
                static_cast<decltype(arg.value)>(~arg.value)};
          }
          this->errors.push_back(
              {RTError("Cannot perform ~ on non-int like", node->op_tok.pos)});
          return VoidValue();
  ...
  if (node->op_tok.type == TokenType::INCREMENT ||
    ...
    NumberVariant new_val = std::move(std::visit(
        [&](auto &n) -> NumberVariant {
          ...
          this->errors.push_back(
              {RTError("Invalid operand for ++/--", pos), "Error"});
          return VoidValue();
        },
        old_val));
  ...
  if (node->op_tok.type == TokenType::AMPERSAND) {
    ...
    if (t_fi >= context->frames.size()) {
      throw RTError("Dangling symbol when taking address of '" + name + "'",
                    pos);
    }
  ...
  if (node->op_tok.type == TokenType::SIZEOF) {
    ...
    if (!str_val) {
      throw RTError("QC-B002: sizeof expects a string argument (type name)",
                    Position("", "", 0, 0, 0));
    }
  ...
  return std::move(std::visit(
      [&node, this](const auto &n) -> NumberVariant {
        ...
        if constexpr (!std::is_same_v<T, StringValue> &&
          ...
          this->errors.push_back(
              {RTError("Unary operator '" + get_token_name(node->op_tok.type) +
  ...
  for (size_t i = 0; i < node->sizes.size(); ++i) {
    if (node->sizes[i].has_value()) {
      if (i >= array_val->sizes().size())
        this->errors.push_back(
            {RTError("Declared array has more dimensions than initialized",
                     node->var_name_tok.pos),
             "Error"});
      if (array_val->sizes()[i] != node->sizes[i].value())
        this->errors.push_back(
            {RTError("Array size mismatch on dimension " + std::to_string(i) +
                         ": declared " +
                         std::to_string(node->sizes[i].value()) + " but got " +
...
NumberVariant Interpreter::operator()(std::unique_ptr<ListDeclNode> &node) {
  ...
  if (std::holds_alternative<std::shared_ptr<ArrayValue>>(init_value)) {
    ...
    auto list_val = std::make_shared<ListValue>(node->type_tok.value,
      ...
      if (auto arr = std::get_if<std::shared_ptr<ArrayValue>>(&spread_val)) {
        for (auto &arr_elem : (*arr)->elements) {
          elements.push_back(arr_elem);
                     ...
                     std::get_if<std::shared_ptr<ListValue>>(&spread_val)) {
        for (auto &list_elem : (*list)->elements) {
          elements.push_back(list_elem);
        }
      } else {
        this->errors.push_back(
            {RTError("Cannot spread non-array type", Position()), "Error"});
      }
    } else {
...
NumberVariant Interpreter::operator()(std::unique_ptr<ArrayAccessNode> &node) {
    ...
    int index = std::visit(
        [&](auto &&arg) -> int {
          ...
          if constexpr (std::is_same_v<T, Number<long double>>)
            return (int)arg.value;
          this->errors.push_back(
              {RTError("Index must be a number", Position()), "Error"});
          return 0;
        },
        index_val);
  ...
  bool is_array =
      std::holds_alternative<std::shared_ptr<ArrayValue>>(base_value);
  ...
  if (is_array)
    current_array = std::get<std::shared_ptr<ArrayValue>>(base_value);
  if (is_list)
    current_list = std::get<std::shared_ptr<ListValue>>(base_value);
  ...
  for (size_t dim = 0; dim < node->indices.size(); ++dim) {
    ...
    if (current_array) {
      ...
      if (auto next_arr = std::get_if<std::shared_ptr<ArrayValue>>(&next)) {
        ...
        current_list.reset();
      } else if (auto next_list =
                     std::get_if<std::shared_ptr<ListValue>>(&next)) {
        current_list = *next_list;
        current_array.reset();
      } else {
        this->errors.push_back(
            {RTError("QC-I003: Cannot index inside non-array/list", Position()),
             "Error"});
      }
...
bool Interpreter::in_class_or_derived_context(const std::string &baseName) {
  ...
  while (it != context->user_types.end() &&
    ...
    if (it->second.baseClassName == baseName)
      return true;
...
NumberVariant Interpreter::operator()(std::unique_ptr<MethodCallNode> &node) {
  ...
  if (auto inst_ptr = std::get_if<std::shared_ptr<InstanceValue>>(&obj)) {
    ...
    if (mname == "has") {
      if (node->args.size() != 1) {
        this->errors.push_back(
            {RTError("QC-BM01: class.has() expects 1 argument (field name)",
    ...
    if (mname == "has_method") {
      if (node->args.size() != 1) {
        this->errors.push_back(
            {RTError(
                 "QC-BM01: class.has_method() expects 1 argument (method name)",
                 node->method_name.pos),
             "Error"});
        return BoolValue("false");
      }
      ...
      if (!std::holds_alternative<StringValue>(arg)) {
        this->errors.push_back(
            {RTError("QC-BM02: class.has_method() argument must be a string",
                     node->method_name.pos),
             "Error"});
        return BoolValue("false");
      }
    ...
    if (!method) {
      this->errors.push_back(
          {RTError("QC-M001: Class '" + className +
                       "' has no method or no matching overload for'" + mname +
                       "'",
                   node->method_name.pos),
           "Error"});
      return VoidValue{};
  ...
  if (auto sv = std::get_if<std::shared_ptr<StructValue>>(&obj)) {
    if (node->method_name.value == "has") {
      if (node->args.size() != 1) {
        this->errors.push_back(
            {RTError("QC-BM01: struct.has() expects 1 argument (field name)",
                     node->method_name.pos),
             "Error"});
        return BoolValue("false");
      }
  ...
  if (auto list_ptr = std::get_if<std::shared_ptr<ListValue>>(&obj)) {
    ...
    if (node->method_name.value == "push") {
      ...
      if (name.find(strip(list->element_type)) == std::string::npos && !loose) {
        this->errors.push_back(
            {RTError("QC-BM02: cannot push a " + name + " to a list of type " +
                         list->element_type,
                     node->method_name.pos),
             "Error"});
    ...
    if (node->method_name.value == "pop") {
      if (node->args.size() != 0) {
        this->errors.push_back({RTError("QC-BM01: pop() takes no arguments",
                                        node->method_name.pos),
                                "Error"});
      }
      return list->pop();
  ...
  if (auto map_ptr = std::get_if<std::shared_ptr<MapValue>>(&obj)) {
    ...
    if (node->method_name.value == "keys") {
      if (node->args.size() != 0) {
        this->errors.push_back({RTError("QC-BM01: keys() takes no arguments",
                                        node->method_name.pos),
                                "Error"});
      }
...
Interpreter::operator()(std::shared_ptr<PropertyAccessNode> &node) {
    ...
    if (auto varAcc =
      ...
      if (ut_it != context->user_types.end()) {
        ...
        if (!ut.enumEntries.empty()) {
          ...
          this->errors.push_back(
              {RTError("QC-F001: Enum '" + baseName + "' has no member '" +
                           memberName + "'",
  ...
  while (true) {
    ...
    if (auto s = std::get_if<std::shared_ptr<StructValue>>(&obj)) {
      ...
      if (it == (*s)->fields.end()) {
        this->errors.push_back(
            {RTError("QC-F001: Unknown field '" + name + "' on struct '" +
                         (*s)->type_name + "'",
                     node->property_name.pos),
             "Error"});
      }
    ...
    if (auto arr = std::get_if<std::shared_ptr<ArrayValue>>(&obj)) {
      ...
      if (name == "length")
        return Number<int>((*arr)->elements.size());
    }
    ...
    if (auto inst = std::get_if<std::shared_ptr<InstanceValue>>(&obj)) {
      ...
      if (!field_exists_on_class(className, name)) {
        this->errors.push_back({RTError("QC-F001: Unknown field '" + name +
                                            "' on class '" + className + "'",
                                        node->property_name.pos),
                                "Error"});
        return VoidValue{};
      ...
      if (ut_it != context->user_types.end() &&
        ...
        if (access == "private") {
          if (!in_class_context(className)) {
            this->errors.push_back(
                {RTError("QC-A001: Field '" + name + "' of class '" +
                             className + "' is " + access +
    ...
    this->errors.push_back(
        {RTError("QC-F002: Unknown property: " + name, node->property_name.pos),
         "Error"});
  }
...
NumberVariant Interpreter::operator()(std::unique_ptr<ArrayAssignNode> &node) {
  ...
  if (auto ptr = std::get_if<PointerValue>(&base)) {
    if (!ptr->is_heap) {
      this->errors.push_back(
          {RTError(
               "QC-B002: cannot assign through non-heap pointer in interpreter",
               ptr->pos),
           "Error"});
      return VoidValue();
    }
    ...
    int index = std::visit(
        [&](auto &&arg) -> int {
          ...
          this->errors.push_back(
              {RTError("Array index must be an integer", Position()), "Error"});
          return 0;
        },
        idx_val);
    ...
    if (index < 0 || (size_t)index >= (*arr)->elements.size()) {
      this->errors.push_back(
          {RTError("QC-I001: Array index out of bounds", Position()), "Error"});
      return VoidValue();
    }
  ...
  if (auto list_ptr = std::get_if<std::shared_ptr<ListValue>>(&base)) {
    ...
    if (auto idx_num = std::get_if<Number<int>>(&idx_val)) {
      ...
      this->errors.push_back(
          {RTError("List index must be an integer", Position()), "Error"});
    }
  }
...
NumberVariant Interpreter::operator()(std::unique_ptr<RandomCallNode> &node) {
  ...
  this->errors.push_back(
      {RTError("random() takes 0, 1, or 2 arguments", Position()), "Error"});
  return VoidValue();
}
...
NumberVariant Interpreter::operator()(std::unique_ptr<FieldAssignNode> &node) {
  ...
  if (auto inst = std::get_if<std::shared_ptr<InstanceValue>>(&base_val)) {
    ...
    if (ut_it == context->user_types.end() ||
        ut_it->second.kind != UserTypeKind::Class) {
      this->errors.push_back(
          {RTError("Unknown class '" + className + "'", node->field_name.pos),
           "Severe"});
      return VoidValue{};
    }
    ...
    if (!found) {
      this->errors.push_back({RTError("Unknown field '" + fieldName +
                                          "' in class '" + className + "'",
                                      node->field_name.pos),
                              "Error"});
      return VoidValue{};
    ...
    if (access == "private") {
      if (!in_class_context(className)) {
        this->errors.push_back(
            {RTError("Field '" + fieldName + "' of class '" + className +
...
llvm::Type *LLVMCompiler::llvmTypeFor(const std::string &qcType) {
  ...
  if (type == "long double")
    return builder->getDoubleTy();
  if (type == "addr_t")
    return builder->getIntNTy(getPtrSize());
  if (type == "char")
  ...
  if (type == "function" || type == "fn" || type.starts_with(("fn "))) {
    return builder->getPtrTy();
  }
  return builder->getInt32Ty();
}
    ...
    if (info.kind == UserTypeKind::Class) {
      if (!info.baseClassName.empty()) {
        ...
        if (base_it != userTypes.end() && base_it->second.is_final_class) {
          cg_error(Position(), "Cannot inherit from final class '" +
                                   info.baseClassName + "'");
          continue;
        }
      }
  ...
  for (auto &[mapKey, info] : userTypes) {
        ...
        for (auto &method : info.classMethods) {
          for (auto &baseMethod : baseInfo.classMethods) {
            if (baseMethod.name_tok.value == method.name_tok.value &&
                baseMethod.is_final) {
              cg_error(method.name_tok.pos, "Cannot override final method '" +
    ...
    for (auto &method : info.classMethods) {
      ...
      llvm::Function *fn = llvm::Function::Create(
          fnTy, llvm::Function::ExternalLinkage, methodName, module.get());
...
llvm::Value *LLVMCompiler::createJaggedArray(AnyNode &literalNode,
  ...
  if (depth == 0) {
    ...
    if (!createRowFn) {
      ...
      llvm::FunctionType *fnTy = llvm::FunctionType::get(
          voidPtrTy, {builder->getInt32Ty(), builder->getInt32Ty()}, false);
      createRowFn =
    ...
    for (size_t j = 0; j < (*arrLit)->elements.size(); j++) {
      ...
      llvm::Value *elemPtr =
          builder->CreateBitCast(tempAlloc, llvm::PointerType::get(context, 0));
  ...
  for (size_t i = 0; i < (*arrLit)->elements.size(); i++) {
    if (auto subLit = std::get_if<std::unique_ptr<ArrayLiteralNode>>(
            &(*arrLit)->elements[i])) {
      llvm::Value *subArr =
          createJaggedArray((*arrLit)->elements[i], elemTypeCode, depth - 1);
...
llvm::Value *LLVMCompiler::emitExpr(AnyNode &node) {
  if (auto num = std::get_if<NumberNode>(&node)) {
    ...
    case TokenType::FLOAT: {
      std::string t = text;
      if (!t.empty() && (std::tolower(t.back()) == 'f'))
        t.pop_back();
      float v = std::stof(t);
    ...
    if (op == TokenType::RSHIFT) {
      ...
      if (startsWithQIn((*bin)->left_node)) {
        if (auto leftBin =
                std::get_if<std::unique_ptr<BinOpNode>>(&(*bin)->left_node)) {
        ...
        if (!qinFn) {
          ...
          qinFn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                         "qc_qin", module.get());
        }
        ...
        if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
          ...
          if (varTy->isIntegerTy(32)) {
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getInt16Ty(), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_short_int_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getInt64Ty(), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_long_int_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getFloatTy(), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_float_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getDoubleTy(), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_double_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getInt8Ty(), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_char_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getInt1Ty(), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_bool_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
            ...
            if (!fn) {
              auto *fnTy = llvm::FunctionType::get(
                  builder->getIntNTy(2), {llvm::PointerType::get(context, 0)},
                  false);
              fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                          "qc_to_qbool_from_string",
                                          module.get());
            }
            converted = builder->CreateCall(fn, {input});
          }
    ...
    if (lty->isPointerTy()) {
      if (auto varAccess =
              std::get_if<std::unique_ptr<VarAccessNode>>(&(*bin)->left_node)) {
        std::string varName = (*varAccess)->var_name_tok.value;
        llvm::Value *alloc = getVarAddress(varName);
        if (alloc) {
    ...
    if (isEq || isNe) {
      ...
      if (lIsUnion && !rIsUnion) {
        ...
        llvm::BasicBlock *matchBB =
            llvm::BasicBlock::Create(context, "tag_matches", currentFunction);
        llvm::BasicBlock *mismatchBB =
            llvm::BasicBlock::Create(context, "tag_mismatch", currentFunction);
        llvm::BasicBlock *endBB =
            llvm::BasicBlock::Create(context, "cmp_end", currentFunction);
        ...
        if (!info.memberTypeStr.empty()) {
          ...
          if (memberTy->isPointerTy()) {
            ...
            if (!strcmp_fn) {
              ...
              auto *fnTy = llvm::FunctionType::get(builder->getInt1Ty(),
                                                   {i8Ptr, i8Ptr}, false);
              strcmp_fn =
                  llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
            ...
            payloadMatch =
                builder->CreateCall(strcmp_fn, {payload, R}, "payload_str_eq");
          } else {
            llvm::Value *typedPtr = builder->CreateBitCast(
                dataPtr, llvm::PointerType::get(context, 0));
            llvm::Value *payload =
                builder->CreateLoad(memberTy, typedPtr, "union_payload");
      ...
      if (!lIsUnion && rIsUnion) {
        ...
        if (!info.memberTypeStr.empty()) {
          ...
          if (memberTy->isPointerTy()) {
            ...
            payloadMatch =
                builder->CreateCall(strcmp_fn, {L, payload}, "payload_str_eq");
          } else {
            llvm::Value *typedPtr = builder->CreateBitCast(
                dataPtr, llvm::PointerType::get(context, 0));
            llvm::Value *payload =
      ...
      if (lIsUnion && rIsUnion) {
        ...
        llvm::BasicBlock *tagMatchBB =
            llvm::BasicBlock::Create(context, "tags_match", currentFunction);
        llvm::BasicBlock *tagMismatchBB =
            llvm::BasicBlock::Create(context, "tags_mismatch", currentFunction);
        llvm::BasicBlock *endBB =
            llvm::BasicBlock::Create(context, "union_cmp_end", currentFunction);
        ...
        llvm::BasicBlock *defaultBB =
            llvm::BasicBlock::Create(context, "cmp_default", currentFunction);
        llvm::SwitchInst *sw =
        ...
        for (size_t i = 0; i < members.size(); i++) {
          llvm::BasicBlock *caseBB = llvm::BasicBlock::Create(
              context, "cmp_case_" + std::to_string(i), currentFunction);
        ...
        llvm::PHINode *payloadPhi =
            builder->CreatePHI(builder->getInt1Ty(), caseResults.size());
    ...
    if ((*bin)->is_f) {
      std::function<llvm::Value *(llvm::Value *, const Position &)> toString =
        ...
        if (ty->isIntegerTy(32)) {
          ...
          if (!fn) {
            auto *i8Ptr = llvm::PointerType::get(context, 0);
            auto *fnTy =
                llvm::FunctionType::get(i8Ptr, {builder->getInt32Ty()}, false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
        ...
        if (ty->isIntegerTy(16)) {
          ...
          if (!fn) {
            auto *i8Ptr = llvm::PointerType::get(context, 0);
            auto *fnTy =
                llvm::FunctionType::get(i8Ptr, {builder->getInt16Ty()}, false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
        ...
        if (ty->isIntegerTy(64)) {
          ...
          if (!fn) {
            auto *i8Ptr = llvm::PointerType::get(context, 0);
            auto *fnTy =
                llvm::FunctionType::get(i8Ptr, {builder->getInt64Ty()}, false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
        ...
        if (ty->isDoubleTy()) {
          ...
          if (!fn) {
            auto *i8Ptr = llvm::PointerType::get(context, 0);
            auto *fnTy =
                llvm::FunctionType::get(i8Ptr, {builder->getDoubleTy()}, false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
        ...
        if (ty->isIntegerTy(1)) {
          ...
          if (!fn) {
            auto *i8Ptr = llvm::PointerType::get(context, 0);
            auto *fnTy =
                llvm::FunctionType::get(i8Ptr, {builder->getInt1Ty()}, false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
        ...
        if (ty->isIntegerTy(2)) {
          ...
          if (!fn) {
            auto *i8Ptr = llvm::PointerType::get(context, 0);
            auto *fnTy =
                llvm::FunctionType::get(i8Ptr, {builder->getIntNTy(2)}, false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
        ...
        if (ty->isArrayTy()) {
          ...
          llvm::ArrayType *dimsArrTy =
              llvm::ArrayType::get(builder->getInt32Ty(), dimensions.size());
          ...
          for (size_t i = 0; i < dimensions.size(); i++) {
            std::vector<llvm::Value *> indices = {builder->getInt32(0),
                                                  builder->getInt32(i)};
            llvm::Value *dimPtr =
                builder->CreateInBoundsGEP(dimsArrTy, dimsAlloc, indices);
            builder->CreateStore(builder->getInt32(dimensions[i]), dimPtr);
          }
          llvm::Value *arrPtr =
              builder->CreateBitCast(v, llvm::PointerType::get(context, 0));

          std::vector<llvm::Value *> dimsIndices = {builder->getInt32(0),
                                                    builder->getInt32(0)};
          llvm::Value *dimsPtr =
              builder->CreateInBoundsGEP(dimsArrTy, dimsAlloc, dimsIndices);
        ...
        for (auto &[enumName, enumTy] : enumTypes) {
          if (ty == enumTy) {
            ...
            return builder->CreateBitCast(dataPtr,
                                          llvm::PointerType::get(context, 0));
          }
        }
        ...
        for (auto &[unionName, unionTy] : unionTypes) {
          if (ty == unionTy) {
            ...
            llvm::AllocaInst *resultAlloc = createEntryAlloca(
                "fstr_union_result", llvm::PointerType::get(context, 0));
            ...
            for (size_t i = 0; i < members.size(); i++) {
              ...
              if (memberTy->isPointerTy()) {
                ...
                memberVal =
                    builder->CreateLoad(memberTy, typedPtr, "union_member");
              }
            ...
            builder->SetInsertPoint(endBB);
            return builder->CreateLoad(llvm::PointerType::get(context, 0),
                                       resultAlloc, "fstr_union_result");
          }
        }
        if (ty->isPointerTy()) {
          return v;
    ...
    if (lIsUnion && rIsUnion) {
      ...
      llvm::BasicBlock *endBB =
          llvm::BasicBlock::Create(context, "union_op_end", currentFunction);
      ...
      for (size_t i = 0; i < members.size(); i++) {
        llvm::BasicBlock *caseBB = llvm::BasicBlock::Create(
            context, "union_op_case_" + std::to_string(i), currentFunction);
        sw->addCase(builder->getInt32(i), caseBB);
        builder->SetInsertPoint(caseBB);
        ...
        case TokenType::LESS:
          res = isFP ? builder->CreateFCmpOLT(lhsVal, rhsVal)
                     : builder->CreateICmpSLT(lhsVal, rhsVal);
          break;
        case TokenType::MORE:
          res = isFP ? builder->CreateFCmpOGT(lhsVal, rhsVal)
                     : builder->CreateICmpSGT(lhsVal, rhsVal);
          break;
        case TokenType::LESS_EQ:
          res = isFP ? builder->CreateFCmpOLE(lhsVal, rhsVal)
                     : builder->CreateICmpSLE(lhsVal, rhsVal);
          break;
        case TokenType::MORE_EQ:
          res = isFP ? builder->CreateFCmpOGE(lhsVal, rhsVal)
                     : builder->CreateICmpSGE(lhsVal, rhsVal);
          break;
        case TokenType::PLUS:
          res = isFP ? builder->CreateFAdd(lhsVal, rhsVal)
                     : builder->CreateAdd(lhsVal, rhsVal);
          break;
        case TokenType::MINUS:
          res = isFP ? builder->CreateFSub(lhsVal, rhsVal)
                     : builder->CreateSub(lhsVal, rhsVal);
          break;
        case TokenType::MUL:
          res = isFP ? builder->CreateFMul(lhsVal, rhsVal)
                     : builder->CreateMul(lhsVal, rhsVal);
          break;
        case TokenType::DIV:
          res = isFP ? builder->CreateFDiv(lhsVal, rhsVal)
                     : builder->CreateSDiv(lhsVal, rhsVal);
          break;
        case TokenType::AMPERSAND:
        ...
        case TokenType::R_ROT: {
          llvm::Intrinsic::ID id = (op == TokenType::L_ROT)
                                       ? llvm::Intrinsic::fshl
                                       : llvm::Intrinsic::fshr;
          llvm::Function *rotFunc = llvm::Intrinsic::getOrInsertDeclaration(
      ...
      builder->SetInsertPoint(endBB);
      return builder->CreateLoad(resultAlloc->getAllocatedType(), resultAlloc,
                                 "union_op_result");
    }
    if (lIsUnion || rIsUnion) {
      ...
      for (size_t i = 0; i < members.size(); i++) {
        ...
        llvm::Value *typedPtr =
            builder->CreateBitCast(payload, llvm::PointerType::get(context, 0));
        llvm::Value *memberVal =
        ...
        if (lTy != rTy) {
          if (lTy->isDoubleTy() || rTy->isDoubleTy()) {
            ...
            if (!lTy->isFloatTy())
              lhsVal = builder->CreateSIToFP(lhsVal, builder->getFloatTy());
            if (!rTy->isFloatTy())
              rhsVal = builder->CreateSIToFP(rhsVal, builder->getFloatTy());
        ...
        case TokenType::R_ROT: {
          if (isFP) {
            cg_error((*bin)->op_tok.pos,
                     "Rotation not allowed on floating-point union members");
            return nullptr;
          }
          llvm::Intrinsic::ID id = (op == TokenType::L_ROT)
                                       ? llvm::Intrinsic::fshl
    ...
    case TokenType::PLUS:
      ...
      if (lty->isPointerTy() || rty->isPointerTy()) {
        ...
        if (lType.ends_with("*") || lType == "@nullptr") {
          ...
          if (rType != "int") {
            cg_error((*bin)->op_tok.pos,
                     "Pointer arithmetic may only be preformed on ptr lhs and "
                     "int rhs, got " +
                         lType + " and " + rType);
            return nullptr;
          }
          ...
          if (lType != rType) {
            cg_error((*bin)->op_tok.pos,
                     "You can only add string with string, got " + lType +
                         " and " + rType);
            return nullptr;
          }
          llvm::Function *concatFn = module->getFunction("qc_string_concat");
        ...
        cg_error((*bin)->op_tok.pos, "Cannot perform arithmetic on types " +
                                         lType + " + " + " rType");
        return nullptr;
      }
    ...
    case TokenType::MINUS:
      ...
      if (lty->isPointerTy() || rty->isPointerTy()) {
        ...
        if ((lType.ends_with("*") || lType == "@nullptr") &&
          ...
          if (lType != rType) {
            cg_error((*bin)->op_tok.pos,
                     "Pointer arithmetic may only be preformed on the same lhs "
                     "and rhs type, got " +
                         lType + " and " + rType);
            return nullptr;
          }
          ...
          baseType.pop_back();
          llvm::Value *diff =
              builder->CreatePtrDiff(llvmTypeFor(baseType), L, R, "ptr_diff");
        ...
        cg_error((*bin)->op_tok.pos,
                 "Invalid pointer subtraction: " + lType + " - " + rType);
        return nullptr;
      }
      if (lty == builder->getInt1Ty() || rty == builder->getInt1Ty()) {
        cg_error((*bin)->op_tok.pos, "Cannot perform arithmetic on bool types");
    ...
    case TokenType::BITWISE_XOR:
      if (isFloatTy) {
        cg_error((*bin)->op_tok.pos,
                 "Cannot perform bitwise operations on float/double types");
        return nullptr;
      }
      if (lty->isPointerTy() || rty->isPointerTy()) {
        cg_error((*bin)->op_tok.pos,
      ...
      if ((*bin)->op_tok.type == TokenType::PIPE)
    ...
    case TokenType::LOGICAL_RSHIFT:
      ...
      if ((*bin)->op_tok.type == TokenType::RSHIFT)
    ...
    case TokenType::R_ROT:
      ...
      }
      {
        ...
        llvm::Function *rotFunc =
            llvm::Intrinsic::getOrInsertDeclaration(module.get(), id, {lty});
    ...
    case TokenType::NOT_EQ: {
      if (lty->isIntegerTy() && rty->isIntegerTy()) {
        ...
        return (op == TokenType::EQ_TO) ? builder->CreateICmpEQ(L, R, "icmpeq")
                                        : builder->CreateICmpNE(L, R, "icmpne");
      }
      ...
      if (lty->isPointerTy() && rty->isPointerTy()) {
        ...
        if (lType.ends_with("*") || rType.ends_with("*") ||
            lType == "@nullptr" || rType == "@nullptr") {
    ...
    case TokenType::MORE_EQ: {
      ...
      switch (op) {
      case TokenType::LESS:
        return isFloatTy ? builder->CreateFCmpOLT(L, R, "fcmplt")
                         : builder->CreateICmpSLT(L, R, "icmplt");
      case TokenType::MORE:
        return isFloatTy ? builder->CreateFCmpOGT(L, R, "fcmpgt")
                         : builder->CreateICmpSGT(L, R, "icmpgt");
      case TokenType::LESS_EQ:
        return isFloatTy ? builder->CreateFCmpOLE(L, R, "fcmple")
                         : builder->CreateICmpSLE(L, R, "icmple");
      case TokenType::MORE_EQ:
        return isFloatTy ? builder->CreateFCmpOGE(L, R, "fcmpge")
                         : builder->CreateICmpSGE(L, R, "icmpge");
      default:
        break;
      }
    }
            ...
            if (auto nestedArrLit =
              ...
              for (size_t j = 0; j < (*nestedArrLit)->elements.size(); j++) {
                ...
                if (!fieldVal)
                  return nullptr;
                nestedStruct =
                    builder->CreateInsertValue(nestedStruct, fieldVal, j);
              }
    ...
    if (userTypeIt != userTypes.end() &&
      ...
      if (auto call =
              std::get_if<std::unique_ptr<CallNode>>(&(*va)->value_node)) {
        if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
                &(*call)->node_to_call)) {
          ...
          if (calledName == qcType) {
            ...
            if (!ctorMethodName.empty()) {
              ...
              llvm::Function *ctor =
                  findMethodOverload(qcType, ctorMethodName, args);
      ...
      if (auto *existingLocal =
        ...
        if (existingTy != newTy) {
          ...
          std::string uniqueName =
              fullName + ".shadow." + std::to_string(shadowId++);
          ...
          alloc = existingLocal;
                     ...
                     llvm::dyn_cast<llvm::GlobalVariable>(existingAlloc)) {
        llvm::Value *rhs = emitExpr((*va)->value_node);
        if (rhs) {
    ...
    if (auto propAccess =
            std::get_if<std::shared_ptr<PropertyAccessNode>>(&(*asn)->target)) {
        ...
        if (varName == "this" && currentThis && !currentClassName.empty()) {
          ...
          for (size_t i = 0; i < classInfo.classFields.size(); i++) {
            if (classInfo.classFields[i].name == fieldName) {
              ...
              llvm::Value *fieldPtr =
                  builder->CreateStructGEP(classTy, currentThis, fieldIdx + i);
              ...
              if (op != TokenType::EQ) {
                ...
                switch (op) {
                case TokenType::PLUS_EQ:
                  rhsVal = isFloat ? builder->CreateFAdd(oldVal, rhsVal)
                                   : builder->CreateAdd(oldVal, rhsVal);
                  break;
                case TokenType::MINUS_EQ:
                  rhsVal = isFloat ? builder->CreateFSub(oldVal, rhsVal)
                                   : builder->CreateSub(oldVal, rhsVal);
                  break;
                case TokenType::MUL_EQ:
                  rhsVal = isFloat ? builder->CreateFMul(oldVal, rhsVal)
                                   : builder->CreateMul(oldVal, rhsVal);
                  break;
                case TokenType::DIV_EQ:
                  rhsVal = isFloat ? builder->CreateFDiv(oldVal, rhsVal)
                                   : builder->CreateSDiv(oldVal, rhsVal);
                  break;
                case TokenType::MOD_EQ:
                  rhsVal = isFloat ? builder->CreateFRem(oldVal, rhsVal)
                                   : builder->CreateSRem(oldVal, rhsVal);
                  break;
                default:
                  break;
                }
      ...
      if (srcTy != destTy) {
        if (srcTy->isFloatTy() && destTy->isDoubleTy()) {
          ...
          if ((srcBits == 1 || srcBits == 2) && (destBits != srcBits)) {
            cg_error((*asn)->op_tok.pos,
                     "Cannot convert bool/qbool to other integer types");
            return nullptr;
          }
          ...
          if (lhsTypeStr == "void*" || rhsType.ends_with("*") ||
              lhsTypeStr == "@nullptr" || rhsType == "@nullptr") {
    ...
    case TokenType::EQ:
      ...
      break;
    case TokenType::PLUS_EQ:
      newVal = isFloatTy ? builder->CreateFAdd(oldVal, rhsVal, "fadd")
                         : builder->CreateAdd(oldVal, rhsVal, "add");
      break;
    case TokenType::MINUS_EQ:
      newVal = isFloatTy ? builder->CreateFSub(oldVal, rhsVal, "fsub")
                         : builder->CreateSub(oldVal, rhsVal, "sub");
      break;
    case TokenType::MUL_EQ:
      newVal = isFloatTy ? builder->CreateFMul(oldVal, rhsVal, "fmul")
                         : builder->CreateMul(oldVal, rhsVal, "mul");
      break;
    case TokenType::DIV_EQ:
      newVal = isFloatTy ? builder->CreateFDiv(oldVal, rhsVal, "fdiv")
                         : builder->CreateSDiv(oldVal, rhsVal, "sdiv");
      break;
    case TokenType::MOD_EQ:
      newVal = isFloatTy ? builder->CreateFRem(oldVal, rhsVal, "frem")
                         : builder->CreateSRem(oldVal, rhsVal, "srem");
      break;
    default:
      cg_error((*asn)->op_tok.pos, "Unsupported assignment operator.");
      return nullptr;
    }
    if ((*asn)->op_tok.type == TokenType::EQ) {
          ...
          if (classTypes.find(className) != classTypes.end()) {
            ...
            if (opMethod) {
              ...
              llvm::Value *callResult =
                  builder->CreateCall(opMethod, allArgs, "op_assign_tmp");
              return callResult;
            }
          }
        }
    ...
    if ((*unary)->op_tok.type == TokenType::MUL) {
      ...
      if (!type.ends_with("*")) {
        cg_error((*unary)->op_tok.pos,
                 "you can only dereference pointer types, found: " + type);
        return nullptr;
      }
    ...
    for (auto &pair : (*mapLit)->pairs) {
      ...
      llvm::Value *keyPtr =
          builder->CreateBitCast(keyAlloc, llvm::PointerType::get(context, 0));
      llvm::Value *valPtr =
          builder->CreateBitCast(valAlloc, llvm::PointerType::get(context, 0));
    ...
    if ((*arrLit)->elements.empty()) {
      ...
      if (retTy->isPointerTy()) {
        ...
        if (!mallocFn) {
          ...
          mallocFn =
              llvm::Function::Create(mallocTy, llvm::Function::ExternalLinkage,
                                     "malloc", module.get());
        }
    ...
    for (size_t i = 0; i < allElements.size(); i++) {
      std::vector<llvm::Value *> indices = {builder->getInt32(0),
                                            builder->getInt32(i)};
      llvm::Value *elemPtr =
          builder->CreateInBoundsGEP(arrTy, alloc, indices, "arr_elem_ptr");
    ...
    if (auto *varAccess =
            std::get_if<std::unique_ptr<VarAccessNode>>(&call.node_to_call)) {
        ...
        if (funcHasAutoParams(funcDef)) {
          ...
          if (specializedFunctions[funcName].count(sig) == 0) {
            llvm::Function *specializedFn =
                generateSpecializedFunction(funcDef, argTypes, specializedName);
      ...
      if (it != builtins.end()) {
        ...
        if (funcName == "typeof" && !call.arg_nodes.empty()) {
          ...
          for (auto &[unionName, unionTy] : unionTypes) {
            if (argTy == unionTy) {
              llvm::Value *tag =
                  builder->CreateExtractValue(arg, 0, "typeof_tag");
              ...
              if (typeIt != userTypes.end()) {
                ...
                llvm::AllocaInst *resultAlloc = createEntryAlloca(
                    "typeof_result", llvm::PointerType::get(context, 0));
                llvm::SwitchInst *switchInst =
                    builder->CreateSwitch(tag, endBB, members.size());
                for (size_t i = 0; i < members.size(); i++) {
                  llvm::BasicBlock *caseBB = llvm::BasicBlock::Create(
                      context, "typeof_case_" + std::to_string(i),
                      currentFunction);
                  builder->SetInsertPoint(caseBB);
          ...
          for (auto &[enumName, enumTy] : enumTypes) {
            if (argTy == enumTy) {
              ...
              llvm::AllocaInst *resultAlloc = createEntryAlloca(
                  "typeof_result", llvm::PointerType::get(context, 0));
              llvm::SwitchInst *switchInst =
                  builder->CreateSwitch(tag, endBB, entries.size());
              for (size_t i = 0; i < entries.size(); i++) {
                llvm::BasicBlock *caseBB =
                    llvm::BasicBlock::Create(context, "case", currentFunction);
                builder->SetInsertPoint(caseBB);
          ...
          if (auto varAccess =
                  std::get_if<std::unique_ptr<VarAccessNode>>(&argNode)) {
            std::string varName = (*varAccess)->var_name_tok.value;
        ...
        if (funcName == "fclose") {
          ...
          if (!fn) {
            auto *fnTy = llvm::FunctionType::get(
                builder->getVoidTy(), {llvm::PointerType::get(context, 0)},
                false);
            fn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
                                        "qc_fclose", module.get());
          }
        ...
        if (funcName == "random" && !call.arg_nodes.empty()) {
          ...
          std::vector<AnyNode> goodArgs(
              std::make_move_iterator(call.arg_nodes.begin()),
          ...
          if (auto *GV = llvm::dyn_cast<llvm::GlobalVariable>(argVal)) {
            if (GV->hasInitializer()) {
              constArray = llvm::dyn_cast<llvm::ConstantDataSequential>(
                  GV->getInitializer());
            }
          } else {
          ...
          if (constArray && constArray->isString()) {
            ...
            cg_error((*varAccess)->var_name_tok.pos,
                     "qout requires the first argument to be a string: " +
                         funcName);
            return nullptr;
          }
          std::string to_print = "";
          ...
          if (!printString) {
            ...
            printString = llvm::Function::Create(
                prStrFnTy, llvm::Function::ExternalLinkage, "qc_print_string",
                module.get());
          }
          ...
          if (!fmtStr) {
            llvm::FunctionType *prStrFnTy = llvm::FunctionType::get(
                ...
                {llvm::PointerType::get(context, 0), builder->getInt32Ty(),
                 builder->getInt1Ty()},
                false);
            fmtStr = llvm::Function::Create(prStrFnTy,
          ...
          if (!fmtInt) {
            llvm::FunctionType *fmtIntFnTy = llvm::FunctionType::get(
                ...
                false);
            fmtInt = llvm::Function::Create(fmtIntFnTy,
                                            llvm::Function::ExternalLinkage,
          ...
          if (!fmtFloat) {
            llvm::FunctionType *fmtFloatFnTy = llvm::FunctionType::get(
                ...
                {builder->getDoubleTy(), builder->getInt32Ty(),
                 builder->getInt32Ty(), builder->getInt32Ty()},
                false);
            fmtFloat = llvm::Function::Create(fmtFloatFnTy,
                                              llvm::Function::ExternalLinkage,
          ...
          if (!fmtDouble) {
            llvm::FunctionType *fmtDoubleFnTy = llvm::FunctionType::get(
                ...
                {builder->getDoubleTy(), builder->getInt32Ty(),
                 builder->getInt32Ty(), builder->getInt32Ty()},
                false);
            fmtDouble = llvm::Function::Create(fmtDoubleFnTy,
                                               llvm::Function::ExternalLinkage,
          ...
          if (!fmtChar) {
            llvm::FunctionType *fmtCharFnTy = llvm::FunctionType::get(
                ...
                {builder->getInt8Ty(), builder->getInt32Ty(),
                 builder->getInt32Ty()},
                false);
            fmtChar = llvm::Function::Create(fmtCharFnTy,
                                             llvm::Function::ExternalLinkage,
          ...
          if (!fmtQBool) {
            llvm::FunctionType *fmtQBoolFnTy = llvm::FunctionType::get(
                ...
                {builder->getInt1Ty(), builder->getInt32Ty(),
                 builder->getInt32Ty()},
                false);
            fmtQBool = llvm::Function::Create(fmtQBoolFnTy,
                                              llvm::Function::ExternalLinkage,
          ...
          if (!fmtBool) {
            llvm::FunctionType *fmtBoolFnTy = llvm::FunctionType::get(
                ...
                {builder->getInt8Ty(), builder->getInt32Ty(),
                 builder->getInt32Ty()},
                false);
            fmtBool = llvm::Function::Create(fmtBoolFnTy,
                                             llvm::Function::ExternalLinkage,
          ...
          if (!fmtPtr) {
            llvm::FunctionType *fmtPtrFnTy = llvm::FunctionType::get(
                ...
                {llvm::PointerType::get(context, 0), builder->getInt32Ty(),
                 builder->getInt1Ty()},
                false);
            fmtPtr = llvm::Function::Create(fmtPtrFnTy,
                                            llvm::Function::ExternalLinkage,
          ...
          if (!fmtOctal) {
            llvm::FunctionType *fmtOctalFnTy = llvm::FunctionType::get(
                ...
                {builder->getInt64Ty(), builder->getInt32Ty(),
                 builder->getInt1Ty()},
                false);
            fmtOctal = llvm::Function::Create(fmtOctalFnTy,
                                              llvm::Function::ExternalLinkage,
          ...
          if (!fmtHex) {
            llvm::FunctionType *fmtHexFnTy = llvm::FunctionType::get(
                ...
                {builder->getInt64Ty(), builder->getInt32Ty(),
                 builder->getInt1Ty()},
                false);
            fmtHex = llvm::Function::Create(fmtHexFnTy,
                                            llvm::Function::ExternalLinkage,
          ...
          for (size_t i = 0; i < fmtString.length(); i++) {
            ...
            if (i > fmtString.length() - 1) {
              cg_error((*varAccess)->var_name_tok.pos,
                       "unexpected end of fmt string: " + funcName);
              return nullptr;
            }
            c = fmtString[i];
            bool zero_pad = false;
            ...
            if (std::isdigit(static_cast<unsigned char>(c))) {
              std::string num;
              while (i < fmtString.size() &&
                     std::isdigit(static_cast<unsigned char>(fmtString[i]))) {
                num += fmtString[i];
                i++;
              }
              c = fmtString[i];
              ...
              builder->CreateCall(
                  ...
                  {builder->CreateCall(
                      ...
                      {bigIntSigned,
                       ...
                       llvm::ConstantInt::get(builder->getInt1Ty(),
                                              zero_pad)})});
              break;
            }
            case 'u': {
              ...
              if (!itgVal->getType()->isIntegerTy()) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "%u formater takes an int-like (int, long int, short "
                         "int, addr_t)");
                return nullptr;
              }
              llvm::Type *i64Ty = builder->getInt64Ty();
            ...
            case 's': {
              ...
              if (!stVal) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "Failed to resolve argument for formatter in " +
                             funcName);
                return nullptr;
              }
              llvm::Value *strVal = builder->CreateGlobalString(to_print);
              ...
              if (i >= fmtString.size()) {
                ...
                if (auto structTy =
                        llvm::dyn_cast<llvm::StructType>(stVal->getType())) {
                  if (structTy->hasName()) {
                    std::string className = structTy->getName().str();

                    if (classTypes.find(className) != classTypes.end()) {
                      cg_error((*varAccess)->var_name_tok.pos,
                               "st formater takes a struct instance: " +
                                   funcName);
                      break;
                    } else if (structTypes.find(className) !=
                               structTypes.end()) {
                      llvm::Function *nestedReprFn =
                          module->getFunction(className + "_repr");
                      if (nestedReprFn) {
                        builder->CreateCall(
                            printString,
                            {builder->CreateCall(nestedReprFn, {stVal})});
                      } else {
                        builder->CreateCall(
                            printString,
                            {builder->CreateGlobalString("(unknown struct)")});
                      }
                    } else {
                      cg_error((*varAccess)->var_name_tok.pos,
                               "st formater takes a struct instance: " +
                  ...
                  cg_error((*varAccess)->var_name_tok.pos,
                           "st formater takes a struct instance: " + funcName);
                  return nullptr;
                }
              } else {
                if (!stVal->getType()->isPointerTy()) {
                  cg_error((*varAccess)->var_name_tok.pos,
                           "s formater takes a string: " + funcName);
                  return nullptr;
                }
                builder->CreateCall(
                    printString,
            ...
            case 'f': {
              ...
              if (!floatVal->getType()->isFloatTy()) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "f formater takes a float: " + funcName);
                return nullptr;
              }
              llvm::Value *strVal = builder->CreateGlobalString(to_print);
              builder->CreateCall(printString, {strVal});
              ...
              builder->CreateCall(
                  ...
                  {builder->CreateCall(
                      fmtFloat,
                      {builder->CreateFPExt(floatVal, builder->getDoubleTy()),
                       llvm::ConstantInt::get(builder->getInt32Ty(), width),
                       llvm::ConstantInt::get(builder->getInt32Ty(), precision),
                       llvm::ConstantInt::get(builder->getInt1Ty(),
                                              zero_pad)})});
            ...
            case 'd': {
              ...
              if (!doubVal->getType()->isDoubleTy()) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "d formater takes a double: " + funcName);
                return nullptr;
              }
              llvm::Value *strVal = builder->CreateGlobalString(to_print);
              builder->CreateCall(printString, {strVal});
            ...
            case 'c': {
              ...
              if (i >= fmtString.size()) {
                ...
                if (auto structTy =
                        llvm::dyn_cast<llvm::StructType>(cVal->getType())) {
                  if (structTy->hasName()) {
                    std::string className = structTy->getName().str();
                    ...
                    if (classTypes.find(className) != classTypes.end()) {
                      ...
                      if (reprMethod) {
                        ...
                        builder->CreateCall(
                            printString,
                            {builder->CreateCall(reprMethod, args)});
                      } else {
                  ...
                  cg_error((*varAccess)->var_name_tok.pos,
                           "cs formater takes a class instance: " + funcName);
                  return nullptr;
                }
              } else {
                if (!cVal->getType()->isIntegerTy(8)) {
                  cg_error((*varAccess)->var_name_tok.pos,
                           "c formater takes a char: " + funcName);
                  return nullptr;
                }
                builder->CreateCall(
                    printString,
            ...
            case 'b': {
              ...
              if (!boolVal->getType()->isIntegerTy(1)) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "b formater takes a bool: " + funcName);
                return nullptr;
              }
              llvm::Value *strVal = builder->CreateGlobalString(to_print);
              builder->CreateCall(printString, {strVal});
            ...
            case 'q': {
              ...
              if (!qboolVal->getType()->isIntegerTy(2)) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "q formater takes a qbool: " + funcName);
                return nullptr;
              }
              llvm::Value *strVal = builder->CreateGlobalString(to_print);
              builder->CreateCall(printString, {strVal});
            ...
            case 'p': {
              ...
              if (!ptVal->getType()->isPointerTy()) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "p formater takes a pointer: " + funcName);
                break;
              }
              builder->CreateCall(
                  printString,
            ...
            case 'e': {
              ...
              if (!decimalVal->getType()->isFloatTy() &&
                ...
                cg_error((*varAccess)->var_name_tok.pos,
                         "e formater takes a number: " + funcName);
              }
              if (decimalVal->getType()->isIntegerTy()) {
                decimalVal =
                    builder->CreateSIToFP(decimalVal, builder->getDoubleTy());
              } else if (decimalVal->getType()->isFloatTy()) {
                decimalVal =
                    builder->CreateFPExt(decimalVal, builder->getDoubleTy());
            ...
            case 'a': {
              ...
              builder->CreateCall(printString, {strVal});
              to_print = "";
              if (aTy->isIntegerTy(32) || aTy->isIntegerTy(64) ||
                  aTy->isIntegerTy(16)) {
                builder->CreateCall(
                    printString,
                    {builder->CreateCall(
                ...
                if (structTy->hasName()) {
                  ...
                  if (structTypes.find(className) != structTypes.end()) {
                    ...
                    if (nestedReprFn) {
                      builder->CreateCall(
                          printString,
                          {builder->CreateCall(nestedReprFn, {val})});
                    } else {
                      builder->CreateCall(
                          printString,
                          {builder->CreateGlobalString("(unknown struct)")});
              ...
              auto *varNode = std::get_if<std::unique_ptr<VarAccessNode>>(
                  &goodArgs[current_arg]);
              if ((aTy->isPointerTy() &&
                   std::get_if<StringNode>(&goodArgs[current_arg])) ||
                  (varNode && hasVarType((*varNode)->var_name_tok.value) &&
                   findVarType((*varNode)->var_name_tok.value)->second ==
                       std::string("string"))) {
                builder->CreateCall(
                    printString,
                    {builder->CreateCall(
              ...
              if (aTy->isFloatTy()) {
                builder->CreateCall(
                    ...
                    {builder->CreateCall(
                        fmtFloat,
                        {builder->CreateFPExt(val, builder->getDoubleTy()),
                         llvm::ConstantInt::get(builder->getInt32Ty(), width),
                         llvm::ConstantInt::get(builder->getInt32Ty(),
                                                precision),
                         llvm::ConstantInt::get(builder->getInt1Ty(),
                ...
                if (structTy->hasName()) {
                  ...
                  if (classTypes.find(className) != classTypes.end()) {
                    ...
                    if (reprMethod) {
                      std::vector<llvm::Value *> args;
                      llvm::AllocaInst *temp =
                          createEntryAlloca("temp_repr", aTy);
                      builder->CreateStore(val, temp);
                      args.push_back(temp);
                      builder->CreateCall(
                          printString, {builder->CreateCall(reprMethod, args)});
                    } else {
                      to_print += "(reprless class)";
                    }
                  }
        ...
        if (funcName == "mapped_ptr" && !call.arg_nodes.empty()) {
          ...
          if (!(val->getType()->isIntegerTy(getPtrSize()))) {
            cg_error((*varAccess)->var_name_tok.pos,
                     "arg 1 must be a integer the size of a pointer (" +
                         std::to_string(getPtrSize()) + ") (addr_t or " +
                         (getPtrSize() == 32 ? "int" : "long long") +
                         "), got a " +
                         std::to_string(val->getType()->getIntegerBitWidth()) +
                         " bit integer (" +
                         ((val->getType()->getIntegerBitWidth() == 32)
                              ? "int"
                              : ((val->getType()->getIntegerBitWidth() == 64)
                                     ? "long int"
                                     : "short int")) +
                         ": " + funcName);
            return nullptr;
        ...
        if (funcName == "to_address" && !call.arg_nodes.empty()) {
          ...
          return builder->CreatePtrToInt(val, builder->getIntNTy(getPtrSize()),
                                         "addr");
        }
        if (funcName == "ternary" && !call.arg_nodes.empty()) {
          ...
          if (is_tr->getType() != is_fl->getType()) {
            cg_error((*varAccess)->var_name_tok.pos,
                     "arg 2 and 3 must be the same type: " + funcName);
            return nullptr;
          }
          llvm::Value *val = emitExpr(call.arg_nodes.front());
        ...
        if (funcName == "inline" && !call.arg_nodes.empty()) {
          ...
          if (data == nullptr) {
            cg_error((*varAccess)->var_name_tok.pos,
                     "arg 1 must be a compile-time string: " + funcName);
            return nullptr;
          }
          int outputs = 0;
          int inputs = 0;
...
char kind;
          ...
          for (int i = 0; i < asm_text.length(); i++) {
            char c = asm_text[i];
            if (i == 0 && c == ';') {
              if (!(i + 2 >= asm_text.size())) {
                isATT = asm_text[i + 1] == 'A' && asm_text[i + 2] == 'T' &&
                        asm_text[i + 3] == 'T';
              }
            }
            if (c != '$') {
              ...
              if (asm_text.length() <= i + 1) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "invalid operand placeholder: " + funcName);
                return nullptr;
              }
              finalized += c;
              i++;
              ...
              try {
                if (i >= asm_text.size() || !std::isdigit(asm_text[i])) {
                  cg_error((*varAccess)->var_name_tok.pos,
                           "expected number after $: " + funcName);
                  return nullptr;
                }
                int index = 0;
              ...
              if (i >= asm_text.size()) {
                cg_error((*varAccess)->var_name_tok.pos,
                         "expected operand kind after asm operand index: " +
                             funcName);
                return nullptr;
              }
              ...
              if (kind != 'r' && kind != 'm' && kind != 'i' && kind != 'g') {
                cg_error((*varAccess)->var_name_tok.pos,
                         "invalid asm operand kind: " + funcName);
                return nullptr;
              }
              op.kind = kind;
              if (op.isOutput) {
          ...
          if (clobber_string_node == nullptr) {
            cg_error((*varAccess)->var_name_tok.pos,
                     "final arg must be a compile-time string: " + funcName);
            return nullptr;
          }
          ...
          for (char c : clobber_string) {
            if (c == '~') {
              if (!current_clobber.empty()) {
                clobbers.push_back(current_clobber);
              }
              current_clobber = "~";
            } else {
              current_clobber += c;
          ...
          for (int i = 0; i < output_count; i++) {
            if (!output_indices.contains(i)) {
              cg_error(
                  (*varAccess)->var_name_tok.pos,
                  "output operands must be contiguous starting at index 0");
              return nullptr;
            }
          }
          ...
          for (int idx : input_indices) {
            if (idx < output_count) {
              cg_error((*varAccess)->var_name_tok.pos,
                       "input operands must come after all outputs");
              return nullptr;
            }
          }
          ...
          for (auto &[idx, op] : unique_outputs) {
            ...
            auto &arg_node = *it;
            auto *out_var =
                std::get_if<std::unique_ptr<VarAccessNode>>(&arg_node);
            if (out_var == nullptr) {
              cg_error((*varAccess)->var_name_tok.pos,
            ...
            llvm::Type *out_ty =
                llvmTypeFor(resolveVarType((*out_var)->var_name_tok.value));
        ...
        if (!fn) {
          cg_error((*varAccess)->var_name_tok.pos,
                   "Built-in function not found in runtime: " + runtimeName);
          return nullptr;
        }
      ...
      for (auto &param : fnDef->params) {
        if (paramIdx >= args.size()) {
          if (param.default_value.has_value()) {
            ...
            cg_error(Position(), "Missing required argument at position " +
    ...
    return retTy->isVoidTy() ? nullptr : callInst;
                 ...
                 std::get_if<std::unique_ptr<ArrayAccessNode>>(&node)) {
    ...
    if (ptrTy.ends_with("*") || ptrTy == "@nullptr") {
      ...
      llvm::Value *addr = builder->CreateGEP(
          llvmTypeFor(ptrTy), emitExpr((*arrAcc)->base), value, "ptr_arr_addr");
      return builder->CreateLoad(llvmTypeFor(ptrTy), addr, "ptr_arr_val");
    }
    if (auto varAcc =
            std::get_if<std::unique_ptr<VarAccessNode>>(&(*arrAcc)->base)) {
      ...
      if (hasJaggedArray(name)) {
        ...
        for (size_t i = 0; i < (*arrAcc)->indices.size(); i++) {
          ...
          std::vector<llvm::Value *> indices = {builder->getInt32(0),
                                                builder->getInt32(i)};
          llvm::Value *idxPtr =
              builder->CreateInBoundsGEP(indicesArrTy, indicesAlloc, indices);
        ...
        if (!getFn) {
          ...
          llvm::FunctionType *fnTy = llvm::FunctionType::get(
              voidPtrTy, {voidPtrTy, intPtrTy, builder->getInt32Ty()}, false);
        ...
        std::vector<llvm::Value *> idxIndices = {builder->getInt32(0),
                                                 builder->getInt32(0)};
        llvm::Value *indicesPtr =
            builder->CreateInBoundsGEP(indicesArrTy, indicesAlloc, idxIndices);

        llvm::Value *elemPtr =
            builder->CreateCall(getFn,
        ...
        case 0:
          ...
          break;
        case 1:
          elemTy = builder->getFloatTy();
          break;
        case 2:
          elemTy = builder->getDoubleTy();
          break;
        case 3:
          elemTy = builder->getInt8Ty();
          break;
        case 4:
          elemTy = builder->getInt1Ty();
          break;
        case 5:
          elemTy = builder->getIntNTy(2);
          break;
        case 6:
        ...
        llvm::Value *typedPtr =
            builder->CreateBitCast(elemPtr, llvm::PointerType::get(context, 0));
      ...
      if (hasList(name)) {
        ...
        llvm::Value *elemPtr =
            builder->CreateCall(getFn, {listPtr, indexVal}, "list_elem_ptr");
        ...
        if ((*arrAcc)->indices.size() > 1) {
          for (size_t i = 1; i < (*arrAcc)->indices.size(); i++) {
            ...
            nestedPtr =
                builder->CreateGEP(elemTy, nestedPtr, idx, "nested_elem_ptr");
          }
      ...
      if (hasMap(name)) {
        ...
        if (!getFn) {
          llvm::Type *voidPtrTy = llvm::PointerType::get(context, 0);
          llvm::FunctionType *fnTy =
              llvm::FunctionType::get(voidPtrTy, {voidPtrTy, voidPtrTy}, false);
          getFn = llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
    ...
    return nullptr;
  } else if (auto propAccess =
                 std::get_if<std::shared_ptr<PropertyAccessNode>>(&node)) {
    ...
    if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
      ...
      if (enumIt != enumTypes.end()) {
        ...
        if (memberIt != enumMemberInfo.end()) {
          ...
          enumVal =
              builder->CreateInsertValue(enumVal, builder->getInt32(tag), 0);
    ...
    if (propName == "length") {
      ...
      if (runtimeIt != runtimeArraySizes.end()) {
        return builder->CreateLoad(builder->getInt32Ty(), runtimeIt->second,
                                   "runtime_len");
      }
      ...
      if (!lenFn) {
        llvm::FunctionType *ty = llvm::FunctionType::get(
            builder->getInt32Ty(), {llvm::PointerType::get(context, 0)}, false);
    ...
    if (propName == "size") {
      ...
      if (!sizeFn) {
        llvm::FunctionType *ty = llvm::FunctionType::get(
            builder->getInt32Ty(), {llvm::PointerType::get(context, 0)}, false);
        sizeFn = llvm::Function::Create(ty, llvm::Function::ExternalLinkage,
        ...
        if (locAlloc) {
          ...
          if (auto structTy = llvm::dyn_cast<llvm::StructType>(allocTy)) {
            ...
            if (userTypeIt != userTypes.end() &&
              ...
              if (fieldIdx == -1) {
                cg_error(Position(),
                         "Struct " + structName + " has no field " + propName);
                return nullptr;
              }

              llvm::Value *fieldPtr = builder->CreateStructGEP(
                  structTy, locAlloc, fieldIdx, propName + "_ptr");
              llvm::Type *fieldTy = structTy->getElementType(fieldIdx);
              return builder->CreateLoad(fieldTy, fieldPtr, propName);
            }
          }
        ...
        for (auto &member : unionInfo.members) {
          ...
          if (classTypes.find(resolvedVariant) != classTypes.end()) {
            ...
            if (fieldIdx != -1) {
              ...
              llvm::Value *dataFieldPtr = builder->CreateStructGEP(
                  unionTy, varAlloc, 1, "union_data_ptr");
              llvm::Value *dataPtr =
              ...
              llvm::Type *fieldTy = classTy->getElementType(fieldIdx);
              llvm::Value *fieldPtr =
                  builder->CreateStructGEP(classTy, castedPtr, fieldIdx);
              return builder->CreateLoad(fieldTy, fieldPtr, propName);
            }
          }
          if (structTypes.find(resolvedVariant) != structTypes.end()) {
            ...
            if (fieldIdx != -1) {
              ...
              llvm::Type *fieldTy = structTy->getElementType(fieldIdx);
              llvm::Value *fieldPtr =
                  builder->CreateStructGEP(structTy, castedPtr, fieldIdx);
              return builder->CreateLoad(fieldTy, fieldPtr, propName);
            }
          }
        }
    ...
    if (auto varAccess =
            std::get_if<std::unique_ptr<VarAccessNode>>(&(*methodCall)->base)) {
      if ((*varAccess)->var_name_tok.value == "this" && currentThis &&
          !currentClassName.empty()) {
          ...
          for (size_t methodIdx : autoMethodIndices[currentClassName]) {
            ...
            if (autoMethod.name_tok.value == methodName &&
              ...
              for (auto it = autoMethod.params.begin();
                   it != autoMethod.params.end(); ++it, ++argIdx) {
                if (it->type.value != "auto") {
                  std::string declared = it->type.value;
                  std::string argType =
                      argIdx < argTypes.size() ? argTypes[argIdx] : "";
                  if (declared.ends_with("&"))
                    declared.pop_back();
                  if (argType.ends_with("&"))
                    argType.pop_back();
                  if (argIdx >= argTypes.size() || argType != declared) {
                    paramsMatch = false;
                    break;
                  }
                }
              ...
              if (!specialized) {
                specialized = generateSpecializedMethod(
                    currentClassName, methodIdx, argTypes, specializedName);
              }
        ...
        if (!isAutoMethod) {
          ...
          for (size_t i = 0; i < (*methodCall)->args.size(); i++) {
            ...
            bool isRef =
                methodInfo && methodInfo->params[i].type.value.ends_with("&");
            if (isRef) {
              auto *va = std::get_if<std::unique_ptr<VarAccessNode>>(&argNode);
              if (!va) {
                cg_error(Position(), "L-value required for ref param");
              ...
              args.push_back(getVarAddress((*va)->var_name_tok.value));
            } else {
              llvm::Value *arg = emitExpr(argNode);
              if (!arg)
                return nullptr;
          ...
          for (size_t i = 0; i < args.size(); i++) {
            ...
            if (actual->getType() != expected) {
              if (!expected->isPointerTy() &&
                ...
                reconciledArgs.push_back(builder->CreateLoad(expected, actual));
              } else {
          ...
          if (autoMethodIndices.find(structName) != autoMethodIndices.end()) {
            for (size_t methodIdx : autoMethodIndices[structName]) {
              ...
              if (autoMethod.name_tok.value == methodName &&
                ...
                if (!canAccessMethod(currentClassName, methodOwnerClass,
                                     methodName)) {
                  cg_error(
                      Position(),
                      "Cannot access private/protected method in this context");
                  return nullptr;
                }
                ...
                if (!specialized) {
                  specialized = generateSpecializedMethod(
                      structName, methodIdx, argTypes, specializedName);
                }
          ...
          if (isAutoMethod && specialized) {
            ...
            for (auto &argNode : (*methodCall)->args) {
              ...
              if (ptype.ends_with("&")) {
                ...
                allArgs.push_back(getVarAddress((*va)->var_name_tok.value));
              } else {
                allArgs.push_back(emitExpr(argNode));
              }
              argIdx++;
          ...
          if (!isAutoMethod) {
            ...
            for (size_t i = 0; i < (*methodCall)->args.size(); i++) {
              ...
              if (isRef) {
                ...
                methodArgs.push_back(getVarAddress((*va)->var_name_tok.value));
              } else {
                llvm::Value *arg = emitExpr(argNode);
                if (!arg)
                  return nullptr;
            ...
            llvm::Function *method =
                findMethodOverload(structName, methodName, methodArgs);
            if (!method) {
              cg_error(Position(), "No matching overload for " + methodName);
              return nullptr;
            }
            ...
            if (auto varAcc = std::get_if<std::unique_ptr<VarAccessNode>>(
                    &(*methodCall)->base)) {
              thisPtr = getVarAddress((*varAcc)->var_name_tok.value);
            } else {
              llvm::AllocaInst *temp = createEntryAlloca("temp_this", baseTy);
              builder->CreateStore(baseVal, temp);
              thisPtr = temp;
    ...
    if (auto varAcc =
            std::get_if<std::unique_ptr<VarAccessNode>>(&(*methodCall)->base)) {
      llvm::Value *varAlloc = getVarAddress((*varAcc)->var_name_tok.value);
      if (varAlloc) {
        llvm::Type *allocTy = getPointeeType((*varAcc)->var_name_tok.value);
        for (auto &[unionName, unionTy] : unionTypes) {
          if (allocTy == unionTy) {
            auto &unionInfo = userTypes[unionName];
            for (auto &member : unionInfo.members) {
              std::string resolvedVariant = resolveTypeName(member.type);
              if (classTypes.find(resolvedVariant) != classTypes.end()) {
                if (classMethods[resolvedVariant].find(methodName) !=
                  ...
                  llvm::Function *method = findMethodOverload(
                      resolvedVariant, methodName, methodArgs);
                  if (!method) {
                    cg_error(Position(),
    ...
    } else if (auto propAcc = std::get_if<std::shared_ptr<PropertyAccessNode>>(
                   &(*methodCall)->base)) {
      if (auto varBase =
              std::get_if<std::unique_ptr<VarAccessNode>>(&*(*propAcc)->base)) {
      ...
      if (!pushFn) {
        llvm::FunctionType *ty = llvm::FunctionType::get(
            ...
            {llvm::PointerType::get(context, 0),
             llvm::PointerType::get(context, 0), builder->getInt32Ty()},
            false);
        pushFn = llvm::Function::Create(ty, llvm::Function::ExternalLinkage,
    ...
    if (methodName == "pop") {
      ...
      if (!popFn) {
        llvm::FunctionType *ty = llvm::FunctionType::get(
            llvm::PointerType::get(context, 0),
            {llvm::PointerType::get(context, 0)}, false);
        popFn = llvm::Function::Create(ty, llvm::Function::ExternalLinkage,
    ...
    return nullptr;
                 ...
                 std::get_if<std::unique_ptr<FieldAssignNode>>(&node)) {
    std::string fieldName = (*fieldAssign)->field_name.value;
    std::string targetTypeStr = "";
    if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
            &(*fieldAssign)->base)) {
      if ((*varAccess)->var_name_tok.value == "this" &&
          !currentClassName.empty()) {
    ...
    if (auto arrLit = std::get_if<std::unique_ptr<ArrayLiteralNode>>(
      ...
      if ((*arrLit)->elements.empty() && targetTypeStr.find("list<") == 0) {
        ...
        valueVal =
            builder->CreateCall(createListFn, {builder->getInt32(typeCode)});
      }
    }
    if (!valueVal) {
    ...
    if (auto propPtr =
            std::get_if<std::shared_ptr<PropertyAccessNode>>(&tempVariant)) {
    ...
    Position pos;*/
    std::string fullName =
        (getCurrentNamespace().empty() ? "" : getCurrentNamespace() + "::") +
    ...
    if (resolveGlobal(ref->target_tok.value) != nullptr) {
      cg_error(ref->target_tok.pos,
               ref->target_tok.value +
                   " is a global. You cannot create references to globals "
      ...
      if (varT.ends_with("&")) {
        cg_error(ref->target_tok.pos,
                 ref->target_tok.value + " is a reference, and you cannot "
...
llvm::Value *LLVMCompiler::convertToString(llvm::Value *val, AnyNode &expr) {
  ...
  if (!fn) {
    ...
    fn = llvm::Function::Create(fty, llvm::Function::ExternalLinkage, fnName,
                                module.get());
  }
...
llvm::Value *LLVMCompiler::emitSpreadFunctionCall(llvm::Value *calleeVal,
  ...
  llvm::AllocaInst *argsArray = builder->CreateAlloca(
      llvm::PointerType::get(context, 0), totalArgsCount, "spread_args_array");
  ...
  for (auto &argNode : call.arg_nodes) {
    if (auto spread = std::get_if<std::unique_ptr<SpreadNode>>(&argNode)) {
      llvm::Value *collVal = emitExpr((*spread)->expr);
      currentIndex = expandSpreadIntoArrays(collVal, (*spread)->expr, argsArray,
                                            typesArray, currentIndex);
    } else {
      ...
      llvm::Value *slot = builder->CreateGEP(llvm::PointerType::get(context, 0),
                                             argsArray, currentIndex);
      builder->CreateStore(argPtr, slot);

      llvm::Value *typeSlot =
          builder->CreateGEP(builder->getInt32Ty(), typesArray, currentIndex);
  ...
  if (!spreadFn) {
    ...
    llvm::FunctionType *ty = llvm::FunctionType::get(
        builder->getVoidTy(),
        {voidPtrTy, builder->getInt32Ty(), llvm::PointerType::get(context, 0),
         llvm::PointerType::get(context, 0), builder->getInt32Ty()},
        false);
  ...
  if (retTy->isVoidTy()) {
    builder->CreateCall(spreadFn, {funcPtr, totalArgsCount, argsArray,
                                   typesArray, builder->getInt32(retTypeCode),
    ...
    llvm::Value *retPtr =
        builder->CreateBitCast(retAlloc, llvm::PointerType::get(context, 0));
      ...
      if (locAlloc) {
        ...
        if (allocTy->isPointerTy()) {
          ...
          std::vector<llvm::Value *> indices = {builder->getInt32(0),
                                                builder->getInt32(0)};
          actualCollVal =
              builder->CreateInBoundsGEP(allocTy, locAlloc, indices, "arr_ptr");
        }
      }
    }
  }
...
llvm::Value *LLVMCompiler::expandSpreadIntoArrays(llvm::Value *collVal,
  ...
  llvm::Value *argSlot = builder->CreateGEP(llvm::PointerType::get(context, 0),
                                            argsArray, currentIdx);
...
LLVMCompiler::createRuntimeSizedArray(std::vector<AnyNode> &elements,
  ...
  for (auto &elem : elements) {
    if (auto spread = std::get_if<std::unique_ptr<SpreadNode>>(&elem)) {
      if (auto varAccess =
              std::get_if<std::unique_ptr<VarAccessNode>>(&(*spread)->expr)) {
        std::string collName =
            resolveTypeName((*varAccess)->var_name_tok.value);
        if (hasList(collName)) {
          auto listIt = findList(collName);
  ...
  for (auto &elem : elements) {
    if (auto spread = std::get_if<std::unique_ptr<SpreadNode>>(&elem)) {
      llvm::Value *collVal = emitExpr((*spread)->expr);
      currentIndex = copySpreadToArray(collVal, (*spread)->expr, arrPtr,
...
llvm::AllocaInst *LLVMCompiler::createEntryAlloca(const std::string &name,
  ...
  if (!currentFunction) {
    ...
    llvm::GlobalVariable *gv = new llvm::GlobalVariable(
        *module, ty, false, llvm::GlobalValue::ExternalLinkage, initVal, name);
...
llvm::Function *LLVMCompiler::emitFuncDef(const FuncDefNode &fn) {
  ...
  for (auto &arg : func->args()) {
    ...
    if (param.signature.has_value()) {
      ...
      lambdaTypes[param.name.value] = llvmFuncTypeFor(
          param.signature->return_types, param.signature->params);
    } else {
      ...
      if (t.find("list<") == 0) {
        ...
        maps[param.name.value] =
            std::make_pair(getTypeCode(key), getTypeCode(val));
        varTypes[param.name.value] = param.type.value;
...
void LLVMCompiler::emitStmt(AnyNode &node) {
  if (std::holds_alternative<std::unique_ptr<VarAssignNode>>(node) ||
        ...
        if (alloc) {
          ...
          if (allocatedTy->isArrayTy()) {
            val = builder->CreateBitCast(
                alloc, llvm::PointerType::get(context, 0), "array_ret_ptr");
          }
        }
      }
      if (auto call =
              std::get_if<std::unique_ptr<CallNode>>(&(*mret)->values[i])) {
        if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
                &(*call)->node_to_call)) {
          std::string funcName = (*varAccess)->var_name_tok.value;
          ...
          if (classTypes.find(funcName) != classTypes.end()) {
            ...
            if (ctor) {
              llvm::AllocaInst *retVal =
                  createEntryAlloca("mret_val_" + std::to_string(i), classTy);
        ...
        if (destTy->isPointerTy() && srcTy->isArrayTy()) {
          ...
          if (!mallocFn) {
            llvm::FunctionType *mallocTy =
                llvm::FunctionType::get(builder->getPtrTy(), {i64Ty}, false);
            mallocFn = llvm::Function::Create(mallocTy,
                                              llvm::Function::ExternalLinkage,
                                              "malloc", module.get());
          }
          ...
          if (ctor) {
            ...
            for (unsigned i = 0; i < classTy->getNumElements(); i++) {
              std::vector<llvm::Value *> indices = {builder->getInt32(0),
                                                    builder->getInt32(i)};
              llvm::Value *fieldPtr =
                  builder->CreateInBoundsGEP(classTy, retVal, indices);
    ...
    if (auto arrayLit =
            std::get_if<std::unique_ptr<ArrayLiteralNode>>(&(*ret)->value)) {
      if (destTy->isPointerTy() && srcTy->isArrayTy()) {
        llvm::ArrayType *arrayType = llvm::cast<llvm::ArrayType>(srcTy);
        llvm::Type *i64Ty = builder->getInt64Ty();
        llvm::Value *size = llvm::ConstantInt::get(
    ...
    for (auto &[enumName, enumTy] : enumTypes) {
      if (cond->getType() == enumTy) {
        ...
        llvm::Value *typedPtr =
            builder->CreateBitCast(dataPtr, llvm::PointerType::get(context, 0));
    ...
    llvm::BasicBlock *nextBB =
        elifBlocks.empty() ? (elseBB ? elseBB : mergeBB) : elifBlocks[0].first;
    ...
    for (auto &section : (*switch_node)->sections) {
      llvm::BasicBlock *bb =
          llvm::BasicBlock::Create(context, "switch.case", currentFunction);
    ...
    for (size_t i = 0; i < (*qif_node)->qelif_branches.size(); i++) {
      ...
      llvm::BasicBlock *elifBodyBB =
          llvm::BasicBlock::Create(context, "qelif.body", currentFunction);
    ...
    if ((*qif_node)->qelse_branch) {
      ...
      llvm::BasicBlock *qelseBodyBB =
          llvm::BasicBlock::Create(context, "qelse.body", currentFunction);
    ...
    llvm::BasicBlock *check_true =
        llvm::BasicBlock::Create(context, "qsw.check_true", currentFunction);
    llvm::BasicBlock *check_false =
        llvm::BasicBlock::Create(context, "qsw.check_false", currentFunction);
    ...
    if ((*qsw)->case_t) {
      case_t_block =
          llvm::BasicBlock::Create(context, "qsw.case_t", currentFunction);
    }
    if ((*qsw)->case_f) {
      case_f_block =
          llvm::BasicBlock::Create(context, "qsw.case_f", currentFunction);
    }
    if ((*qsw)->case_n) {
      case_n_block =
          llvm::BasicBlock::Create(context, "qsw.case_n", currentFunction);
    }
    if ((*qsw)->case_b) {
      case_b_block =
          llvm::BasicBlock::Create(context, "qsw.case_b", currentFunction);
    }
    ...
    llvm::Value *is_false =
        builder->CreateICmpNE(has_false, builder->getIntN(2, 0), "is_false");
    ...
    llvm::BasicBlock *check_qtrue =
        llvm::BasicBlock::Create(context, "qsw.check_qtrue", currentFunction);
    llvm::BasicBlock *check_qfalse =
        llvm::BasicBlock::Create(context, "qsw.check_qfalse", currentFunction);
    llvm::BasicBlock *check_none_final = llvm::BasicBlock::Create(
        context, "qsw.check_none_final", currentFunction);
    builder->CreateCondBr(is_both, case_b_block ? case_b_block : qswitch_end,
                          check_qtrue);
    builder->SetInsertPoint(check_qtrue);
    builder->CreateCondBr(
        is_qtrue_only, case_t_block ? case_t_block : qswitch_end, check_qfalse);
    ...
    builder->CreateCondBr(is_none, case_n_block ? case_n_block : qswitch_end,
                          qswitch_end);
  ...
  } else if (auto arrDecl =
                 std::get_if<std::unique_ptr<ArrayDeclNode>>(&node)) {
    ...
    if (auto arrLit = std::get_if<std::unique_ptr<ArrayLiteralNode>>(
      ...
      if (isJagged) {
        ...
        llvm::Value *jaggedArr =
            createJaggedArray((*arrDecl)->value, elemTypeCode, depth - 1);
        llvm::Type *ptrTy = llvm::PointerType::get(context, 0);
        llvm::AllocaInst *alloc = createEntryAlloca(name, ptrTy);
          ...
          for (size_t i = 0; i < (*lit)->elements.size(); i++) {
            ...
            if (std::holds_alternative<std::unique_ptr<ArrayLiteralNode>>(
              ...
              std::vector<llvm::Value *> llvmIndices =
                  useHeap ? std::vector<llvm::Value *>{builder->getInt32(0)}
              ...
              llvm::Value *elemPtr =
                  builder->CreateInBoundsGEP(arrTy, basePtr, llvmIndices);
              builder->CreateStore(elemVal, elemPtr);
            }
    ...
    return;
  } else if (auto arrAssign =
                 std::get_if<std::unique_ptr<ArrayAssignNode>>(&node)) {
    if (auto arrAcc = std::get_if<std::unique_ptr<ArrayAccessNode>>(
            &(*arrAssign)->array_access)) {
      std::string ptrTy = getExpressionType((*arrAcc)->base);
      if (ptrTy.ends_with("*") || ptrTy == "@nullptr") {
        if (ptrTy == "@nullptr") {
          cg_error(Position(), "Attempted to dereference nullptr");
        ...
        ptrTy.pop_back();
        llvm::Value *addr =
            builder->CreateGEP(llvmTypeFor(ptrTy), emitExpr((*arrAcc)->base),
                               value, "ptr_arr_asi");
      ...
      if (auto varAcc =
        ...
        if (hasJaggedArray(name)) {
          ...
          llvm::Value *jaggedPtr = builder->CreateLoad(
              llvm::PointerType::get(context, 0), it->second, "jagged_ptr");
          llvm::ArrayType *indicesArrTy = llvm::ArrayType::get(
              builder->getInt32Ty(), (*arrAcc)->indices.size());
          llvm::AllocaInst *indicesAlloc =
              createEntryAlloca("indices_arr", indicesArrTy);
        ...
        if (hasMap(name)) {
          ...
          llvm::Value *mapPtr = builder->CreateLoad(
              llvm::PointerType::get(context, 0), it->second, "map_ptr");
        ...
        if (hasList(name)) {
          ...
          if (!setFn) {
            ...
            llvm::FunctionType *fnTy = llvm::FunctionType::get(
                builder->getVoidTy(),
                {voidPtrTy, builder->getInt32Ty(), voidPtrTy}, false);
            setFn =
                llvm::Function::Create(fnTy, llvm::Function::ExternalLinkage,
    ...
    return;
  } else if (auto listDecl =
                 std::get_if<std::unique_ptr<ListDeclNode>>(&node)) {
    ...
    if (auto callNode =
      ...
      for (auto &elem : (*arrLit)->elements) {
        if (auto spread = std::get_if<std::unique_ptr<SpreadNode>>(&elem)) {
          llvm::Value *collVal = emitExpr((*spread)->expr);
          expandSpreadIntoList(collVal, (*spread)->expr, listPtr, pushFn,
                               elemTypeCode);
        } else {
          llvm::Value *elemVal = emitExpr(elem);
          if (!elemVal)
          ...
          builder->CreateCall(
              pushFn, {listPtr, elemPtr, builder->getInt32(elemTypeCode)});
        }
      }
    } else if (auto methodCall = std::get_if<std::unique_ptr<MethodCallNode>>(
                   &(*listDecl)->value)) {
      listPtr = emitExpr((*listDecl)->value);
      if (!listPtr)
        return;
    ...
    if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
      ...
      if (alloc) {
        ...
        if (allocTy->isArrayTy()) {
          ...
          } else if (runtimeArraySizes.find(collName) !=
                     runtimeArraySizes.end()) {
            isArray = true;
            arrayAlloc = alloc;
            llvm::AllocaInst *sizeAlloc = runtimeArraySizes[collName];
            lengthVal = builder->CreateLoad(builder->getInt32Ty(), sizeAlloc,
                                            "runtime_len");
            if (hasArrayType(collName)) {
              auto typeIt = findArrayType(collName);
              arrayElemTy = llvmTypeFor(typeIt->second);
    ...
    if (isArray && arrayAlloc) {
      ...
      if (allocTy->isArrayTy()) {
        ...
        llvm::Value *elemPtr = builder->CreateInBoundsGEP(allocTy, arrayAlloc,
                                                          indices, "elem_ptr");
        ...
        llvm::Value *elemPtr =
            builder->CreateGEP(arrayElemTy, heapPtr, iVal, "heap_elem_ptr");
...
std::pair<bool, int> LLVMCompiler::checkJagged(AnyNode &node) {
  if (auto lit = std::get_if<std::unique_ptr<ArrayLiteralNode>>(&node)) {
    ...
    size_t firstSize = 0;
    if (auto firstLit = std::get_if<std::unique_ptr<ArrayLiteralNode>>(
            &(*lit)->elements[0])) {
    ...
    for (auto &elem : (*lit)->elements) {
      ...
      if (auto elemLit =
              std::get_if<std::unique_ptr<ArrayLiteralNode>>(&elem)) {
...
std::vector<CTError> LLVMCompiler::compile(StatementsNode *root,
    ...
    for (auto &decl : ns.body) {
      if (auto va = std::get_if<std::unique_ptr<VarAssignNode>>(&decl)) {
        ...
        varTypes[fullName] = (*va)->type_tok.value;
                     ...
                     std::get_if<std::unique_ptr<NamespaceNode>>(&decl)) {
  ...
  for (auto &[className, info] : userTypes) {
    ...
    for (size_t methodIdx = 0; methodIdx < info.classMethods.size();
      ...
      bool hasAutoReturn = !method.return_types.empty() &&
                           method.return_types[0].value == "auto";
            ...
            if (auto varAccess = std::get_if<std::unique_ptr<VarAccessNode>>(
              ...
              if (callName == info.baseClassName) {
                ...
                llvm::Function *parentCtor = findMethodOverload(
                    info.baseClassName, info.baseClassName, parentArgs);
                if (parentCtor) {
                  std::vector<llvm::Value *> allArgs = {currentThis};
                  allArgs.insert(allArgs.end(), parentArgs.begin(),
                                 parentArgs.end());
                  builder->CreateCall(parentCtor, allArgs);
                  bodyStartIdx = 1;
                } else {
                  cg_error(Position(), "Parent class '" + info.baseClassName +
...
std::string collapse_inline_brackets(const std::string &s) {
  ...
  for (size_t i = 0; i < s.size(); ++i) {
    if (s[i] == '[') {
      ...
      bool prev_is_value_starter = (prev == '=' || prev == '(' || prev == ',' ||
                                    prev == '{' || prev == '[');
  ...
  build = [&](const Node &n) -> ASCIINode {
    ...
    for (int i = 0; i < max_child_height; i++) {
      ...
      for (size_t j = 0; j < child_nodes.size(); j++) {
        ...
        std::string part =
            (i < (int)c.lines.size()) ? c.lines[i] : std::string(c.width, ' ');
...
Mer run(std::string file, std::string text, RunConfig config = {}) {
  ...
  try {
    ...
    return Mer{
        Aer{nullptr, nullptr},
        Ler{std::vector<Token>{}, std::make_unique<InvalidSyntaxError>(
  ...
  if (config.compile_mode) {
    ...
    std::string base_name = config.output_file.empty()
                                ? "out"
                                : removeExtension(config.output_file);
    size_t last_slash = base_name.find_last_of("/\\");
    std::string dir = (last_slash == std::string::npos)
                          ? ""
                          : base_name.substr(0, last_slash + 1);
    std::string stem = (last_slash == std::string::npos)
                           ? base_name
                           : base_name.substr(last_slash + 1);

    std::string ll_file =
        config.compile_only ? base_name + ".ll" : dir + "temp_" + stem + ".ll";
    std::string obj_file =
    ...
    if (llc_result != 0) {
      diagnostics.push_back({RTError("Failed to compile IR to object file",
                                     Position("", "", 0, 0, 0)),
                             "Error"});
      return Mer{std::move(ast), std::move(resp), message, diagnostics};
    }
...
Token Lexer::make_number() {
  ...
  if (is_hex) {
    while (this->current_char != '\0' &&
           isCharInSet(this->current_char, DIGITS + "abcdefABCDEF")) {
    ...
    while (this->current_char != '\0' && std::isdigit(this->current_char) &&
           this->current_char - '0' < 8) {
      num += this->current_char;
      this->advance();
    }
    ...
    while (this->current_char != '\0' &&
           isCharInSet(this->current_char, DIGITS + ".f")) {
...
Token Lexer::make_identifier() {
  ...
  if (id == "int" || id == "float" || id == "double" || id == "bool" ||
      id == "case" || id == "string" || id == "qbool" || id == "void" ||
      id == "char" || id == "break" || id == "if" || id == "else" ||
      id == "while" || id == "for" || id == "switch" || id == "return" ||
      id == "qif" || id == "qelse" || id == "qelif" || id == "qswitch" ||
...
Token Lexer::make_string() {
  ...
  while (this->current_char != '\0' &&
         (this->current_char != '"' || escape_character)) {
    if (escape_character) {
...
Token Lexer::make_fstring() {
  ...
  if (this->current_char != '"')
    throw IllegalCharError("QC-IC02: Unterminated f-string", this->pos);
  this->advance();
  std::string encoded = "";
  for (size_t i = 0; i < parts.size(); i++) {
    encoded += parts[i];
    if (i < exprs.size())
      encoded += "\x01" + exprs[i] + "\x01";
  }
  ...
  while (this->current_char != '\0') {
    ...
    if (isCharInSet(this->current_char, bad_chars)) {
      ...
      default:
        ...
        return Ler{std::vector<Token>(), std::make_unique<IllegalCharError>(
...
std::string extract_namespace(const std::string &source,
  ...
  if (pos == std::string::npos) {
    throw std::runtime_error("QC-IX02: Namespace '" + ns_name +
                             "' not found in file");
  }
  size_t brace_start = source.find('{', pos);
  if (brace_start == std::string::npos) {
    throw std::runtime_error("QC-N003: Invalid namespace syntax for '" +
                             ns_name + "'");
  }
  int depth = 1;
  ...
  if (depth != 0) {
    throw std::runtime_error("QC-N002: Unmatched braces in namespace '" +
                             ns_name + "'");
  }
  ...
  process_file = [&](const std::string &file_path,
    ...
    if (!included_namespaces.count(exported_key)) {
      ...
      for (size_t scan = 0; scan < file_content.size(); scan++) {
        if (file_content[scan] == '"' &&
            (scan == 0 || file_content[scan - 1] != '\\')) {
          in_string = !in_string;
        }
        if (!in_string &&
            file_content.substr(scan, 18) == "namespace Exported") {
          exported_pos = scan;
          break;
        }
      }
      ...
      if (exported_pos != std::string::npos) {
        ...
        std::string exported_content =
            file_content.substr(content_start, i - content_start - 1);

        size_t inc_pos = 0;
        while ((inc_pos = exported_content.find("#include", inc_pos)) !=
               std::string::npos) {
          bool in_str = false;
          for (size_t check = 0; check < inc_pos; check++) {
            if (exported_content[check] == '"' &&
                (check == 0 || exported_content[check - 1] != '\\')) {
              in_str = !in_str;
            }
          }
          if (in_str) {
          ...
          std::string directive =
              exported_content.substr(start + 1, end - start - 1);
        ...
        while ((inc_pos = exported_content.find("#include", inc_pos)) !=
          ...
          last = end + 1;
          if (last < exported_content.size() &&
              exported_content[last] == '\n') {
            last++;
          }
          inc_pos = last;
        }
